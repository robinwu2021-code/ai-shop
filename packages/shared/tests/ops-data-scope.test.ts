 ...
 */
if (!MODULES.includes("pay/pay-domain") || !MODULES.includes("job/job-core")
...
const ANCHOR_WAIVED: Record<string, string> = {
  ...
  "pmt_period:COMMUNITY":
    "集单的一期是活动的实例，同 pmt_activity：属于商家，不属于片区。"
...
describe("运营端数据域接入", () => {
  ...
  const OPS_READS_UNREGISTERED_OK: Record<string, string> = {
     ...
     * #repointServiceAreas`（合并社区那条路）是**不带绕过**地扫全平台的 ——
     * 它要把所有指向被合并社区的覆盖项改指过去。登记之后，配了数据域的运营
     * 做一次合并只会改到自己域内商家的那几行，其余的留下一个指向不存在社区的
     * 覆盖项：那家店的范围里凭空少一个小区，零报错。
     * 照着闸门的建议登记，能变绿，会在真库上炸。
     *
     * 而这条读本身不构成越权：门店已经过了 `requireStoreInScope`，
     * 覆盖挂在这家店的主体上，属于「按已授权主键回捞明细」。
     */
    mch_service_area:
      "合并社区（repointServiceAreas）必须扫全平台，登记会让配了域的运营"
      + "合并出一批指向不存在社区的覆盖项且零报错；这条 ops 读则是门店过了"
      + "requireStoreInScope 之后的明细回捞，不构成越权",
    /*
     * 2026-09-04 · 坐标健康度（GET /ops/coverage/health）读它。
     */
    usr_address:
      "归属列是 user_no —— 一条收货地址属于**某个人**，而 ops 的三个维度是"
      + "商家 / 片区 / 自提点，没有一个能回答「这条地址归谁管」。"
      + "地址上也没有 community_no（它的归属是按坐标实时算的，见 TDD-位置与经营范围 §7.1.2），"
      + "所以连冗余一列都不成立。"
      + "而 /ops/coverage/health 只取**聚合数**（总数 / 有坐标数），不返回任何明细 ——"
      + "那一页要回答的是「分母有多脏」，不是「谁住哪儿」。"
      + "登记的后果反而是：SELF 会让这个数变成「运营自己有几条地址」，"
      + "而那个数对谁都没有意义。",
    mch_entity_apply:
