...
export const ENUM_REGISTRY: EnumEntry[] = [
  ...
  { decl: "ops-web:Role", dom: "auth", shape: "CLASS", verdict: "RENAME",
    note: "**与后端是两套词**：后端 Perms.ROLE_PERMS 只有 SUPER_ADMIN/BD/GOODS_OPS/SUPPORT 四个，ops-web 有十一个，只有 SUPER_ADMIN 对得上（BD↔MERCHANT_BD、GOODS_OPS↔PRODUCT_OPS、SUPPORT↔CS 是异名同义）。已在 ops-web 的 http 层加映射并加守卫，但**权限码本身仍是两套体系**（后端 order:view vs 前端 order:order:read），前端用自己写死的 ROLE_PERMS 判权 —— 与 LoginResp 注释「权限判定以后端为准」自相矛盾。对齐是单独一批" },
  ...
  { decl: "ops-web:SettleStatus", dom: "finance", shape: "STATUS", verdict: "RENAME",
    note: "【只读对账，不动实现 —— 涉及钱】与后端 StlBill 部分不一致：前三个 PENDING/SPLITTING/SPLIT 相同；ops-web 的 FAILED 在后端被拆成 RETRYING（自动重试中）与 MANUAL（重试用尽转人工），后端更细；ops-web 的 FROZEN_BACK（超时解冻回平台）与后端 REVERSED（退款前回退分账）**语义不同不是同一个态**。shared:SettleBillStatus 已于 2026-08-11 收敛到后端口径，这份是同组的第三方",
  ...
  { decl: "ops-web:PushStatus", dom: "message", shape: "STATUS", verdict: "PLANNED",
    note: "推送任务状态，后端无推送任务",
