...
export const ENUM_REGISTRY: EnumEntry[] = [
  ...
  // 营销域（TDD-营销域-详细设计）：玩法模板与集单的期
  { decl: "shared:ActivityTrigger", dom: "marketing", shape: "CLASS", verdict: "OK",
  ...
  { decl: "shared:RuleField", dom: "marketing", shape: "CLASS", verdict: "OK",
    note: "新建活动「规则」组的行键，不下发、不落库 —— 纯界面结构" },
  { decl: "shared:BatchPeriodStatus", dom: "marketing", shape: "STATUS", verdict: "OK",
    words: ["OPEN", "SHORT", "CONFIRMED"],
    note: "SHORT = 截单时未达起订量、等商家处理；过了处理时限自动转 CANCELLED 并退款" },
  { decl: "shared:ActivityRuleKind", dom: "marketing", shape: "CLASS", verdict: "OK",
    words: ["CONDITION", "BENEFIT"],
    note: "自己组合（原型 s11）一行的种类；与 pmt_activity_rule.kind 同一套取值（服务端内部列，已在 DISMISSED 说明）" },
  { decl: "shared:EnrollmentStatus", dom: "marketing", shape: "STATUS", verdict: "OK",
    words: ["SUBMITTED", "WITHDRAWN"],
    note: "平台活动报名单（另有 L1 的 APPROVED / REJECTED）。审核通过的那一刻带条件占平台预算；只有 SUBMITTED 能审、能撤" },
  ...
  { decl: "shared:SettleBillStatus", dom: "settle", shape: "STATUS", verdict: "OK",
    ...
    words: ["SPLITTING", "SPLIT", "SPLIT_CONFIRMED", "OFFLINE_SETTLED", "RETRYING", "MANUAL", "REVERSED", "PENDING_RECON", "CONFIRMED", "PAID"] },
  { decl: "shared:SettleBatchStatus", dom: "settle", shape: "STATUS", verdict: "MERGE",
    note: "与 ops-web 的 SettleBatchStatus 同名同值 —— 两处声明，取值必须一起改。账期批次的状态：**它与 SettleBillStatus 不是一回事**，批次状态说「流程走到哪」（能不能放），单据状态说「钱在哪」（放得成不成）。合成一个的话，「已对账待放款」和「已到账」在界面上就成了同一句话。",
    words: ["COLLECTED", "RECONCILING", "BLOCKED", "RECONCILED", "RELEASED"] },
  { decl: "ops-web:SettleBatchStatus", dom: "settle", shape: "STATUS", verdict: "MERGE",
    note: "与 shared 的 SettleBatchStatus 同名同值。阶段 1 支付域独立后两处应一起收进支付域契约（ADR-021）",
    words: ["COLLECTED", "RECONCILING", "BLOCKED", "RECONCILED", "RELEASED"] },
  { decl: "shared:DebtTxnType", dom: "settle", shape: "CLASS", verdict: "MERGE",
    note: "与 ops-web 的 DebtTxnType 同名同值 —— 两处声明，取值必须一起改" },
  { decl: "ops-web:DebtTxnType", dom: "settle", shape: "CLASS", verdict: "MERGE",
    note: "与 shared 的 DebtTxnType 同名同值" },
  { decl: "ops-web:ReconScope", dom: "settle", shape: "CLASS", verdict: "OK",
    note: "对账覆盖面。SELF_ONLY 不是「对完了」，只是我方账内自洽 —— 通道账单下载还没接" },
