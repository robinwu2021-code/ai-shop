...
export const ICONS = {
  ...
  /*
   * 扫码：**取景框四角 + 中间一条扫描线**（2026-09-18）。
   *
   * 不复用 `search` 那枚放大镜 —— 挑货弹层里本来就有搜索框，
   * 同一枚图标同时表示「搜」和「扫」会让两件事看起来是一件。
   *
   * 四个角而不是整圈方框：那是取景框的通用画法，也与相机取景的实际样子对得上。
   */
  scan: wrap(
    '<path d="M3.5 8.5v-3a2 2 0 0 1 2-2h3"/><path d="M15.5 3.5h3a2 2 0 0 1 2 2v3"/>'
    + '<path d="M20.5 15.5v3a2 2 0 0 1-2 2h-3"/><path d="M8.5 20.5h-3a2 2 0 0 1-2-2v-3"/>'
    + '<path d="M3.5 12h17"/>',
  ),
  /* 说明：字段旁的小圆圈 i，点开看口径（编辑商品「每人限购」）。常驻的长说明收进它 */
  info: wrap('<circle cx="12" cy="12" r="8.4"/><path d="M12 11v5.2"/><circle cx="12" cy="7.9" r="0.6"/>'),
  ...
  /* 收藏：五角星。空心 = 没收藏，实心 = 收藏着（商品详情标题旁、我的收藏） */
  star: wrap('<path d="m12 3.6 2.6 5.3 5.8.8-4.2 4.1 1 5.8-5.2-2.7-5.2 2.7 1-5.8-4.2-4.1 5.8-.8z"/>'),
  starFilled: wrapFilled('<path d="m12 3.2 2.75 5.57 6.15.9-4.45 4.33 1.05 6.12L12 17.23l-5.5 2.89 1.05-6.12L3.1 9.67l6.15-.9z"/>'),
  /* 已认证：盾牌 + 勾。店名后面跟一个小号的，替掉原先那颗「已认证」文字 chip ——
     店名行上已经有「自营」chip，两颗 chip 夹着店名，名字反而成了最不显眼的那个 */
  verified: wrap('<path d="M12 3 19 6v6c0 4.5-3 7.5-7 9-4-1.5-7-4.5-7-9V6z"/><path d="m9 12 2 2 4-4"/>'),
