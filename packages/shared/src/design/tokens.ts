...
/**
 * 间距 —— **这不是「阶」，是一张 4rpx 网格**。
 *
 * <p><b>2026-09-07 按实际用量重写。</b>此前声明的是五档 `8/16/28/40/64`，
 * 并在《规范·版面》里写着「只用这几档」。而全仓 1292 处间距取值里落在那五档上的
 * 只有 **39%**，用了 28 个不同的数：
 *
 * <pre>
 *   8×223  16×218  20×205  12×177  24×162  4×66  32×49  28×47  40×17  48×12  64×2
 * </pre>
 *
 * <b>`20rpx` 用了 205 次，而档位里的 `64rpx` 只有 2 次。</b>按用量排下去就是
 * 「4 的倍数全都在用」—— 也就是说这套界面的间距体系**本来就是 4rpx 网格**，
 * 「五档」是一句从来没成立过的话。
 *
 * <p>而这个矛盾**早就有人发现了**，只是修错了方向：`check-page-spec.py` 的头注
 * 写着「按五档判会报出 361 处，其中 12rpx×129 / 20rpx×100 全是 4rpx 网格上的
 * 正常值 —— 那是判据说多了」，于是**放松了闸门**，没有回头改这份声明。
 * 结果三把尺各说各的：这里说五档、闸门查网格、文档写「只用这几档」，三者都绿。
 *
 * <p><b>所以：闸门查的是网格（已 100% 合规），这张表只是把网格上真正在用的成员
 * 列出来给人看</b>，不是白名单。想加一个新数之前，先问它和邻居差的那 4rpx
 * 有没有人分得出 —— 多数时候答案是没有，那就用邻居。
 *
 * <p>没有任何代码 import 它（只有 `gen-ui-lib.py` 读去生成文档）。
 * 真正被组件用的是**语义变量**：`--sh-pad-page` / `--sh-pad-card` /
 * `--sh-pad-empty` / `--sh-gap-block` / `--sh-gap-tabs` —— 它们按端有不同的值，
 * 那才是「间距该怎么定」的落点。
 */
