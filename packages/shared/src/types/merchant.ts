...
export interface MerchantProfile {
   ...
   * <b>《商家服务协议》还没人勾</b>（三期）。
   *
   * <p>只在**代填的单子**上为真 —— 运营代商家进件时不能替商户勾协议。
   *
   * <p>⚠️ **不是「agreed_at 为空」**：存量单子这一列同样全是空的
   * （协议勾选此前从没落过库），那样判会让全体存量商家明天一早都看到
   * 一条「你还没同意协议」，而他们当初确实勾过、只是没人记下来。
   * 一条对所有人恒亮的提示等于没有提示。
   *
   * <p>当前**只提示不拦截**：今天没有提现可挂，而拦上架或拦收款会打断
   * 一家已经审核通过的店的生意 —— 协议没勾是平台流程造成的（运营代填）。
   */
...
export interface MerchantApplyStatus {
   ...
   * <b>这张单是运营代填的</b>（三期）。
   *
   * <p>商户首次登录时必须看到这件事 —— 否则他会发现自己名下凭空有一家店，
   * 而资料是谁在什么时候录的无从得知。
   *
   * <p>给的是布尔而不是代填人账号：他要知道的是「这不是我自己填的」，
   * 运营的员工标识不该发给外部商户。要查是谁填的走审计日志。
   */
  onBehalf?: boolean;
  /**
   * 本人同意《商家服务协议》的时刻（毫秒）；<b>0 / 空 = 尚未同意</b>。
   *
   * <p>⚠️ 空<b>不等于</b>「他拒绝了」，也不等于「这是代填单」——
   * 存量单子同样是空的（协议勾选此前从没落过库，`agreed` 传到
   * `LoginCommand` 就断了）。要分开看 {@link onBehalf}。
   */
  agreedAt?: number;
}
