...
export interface LocationContext {
  ...
  coarse: boolean;
  /**
   * 所在**区县**码（6 位）。模糊定位这一级唯一站得住的结论 —— 5 公里误差落不准小区，
   * 但落得准区。拿它当商品池的筛选条件，「位置不明」才不会等于「看全平台的货」。
   *
   * **null 才是空态那一格**：连模糊定位都拒了，这时要位置，而不是列一屏买不到的东西。
   */
  regionCode: string | null;
  /** 「西湖区」。顶栏要说明白「当前按 XX 区在看」，只给一串码等于没说 */
  regionName: string | null;
  /**
   * **没落进任何围栏时**，最近的那个已开通聚落（M6）。端上拿它当默认归属。
   *
   * <p>冷启动期全市只有一两个聚落，「不在围栏里」是**常态**而不是异常，
   * 而那时按区筛几乎总是空的 —— 首页就空着。
   *
   * <p>`null` 有两种：超出上限（够不着，给了也是让人看一屏送不到的货），
   * 或者已经落进围栏（那时 `innermostNo` 就是答案，两个主语迟早会被选错）。
   */
  nearestNo: string | null;
  /** 顶栏直接显示 */
  nearestName: string | null;
  /**
   * 到最近那个聚落的米数。**超上限时仍然给** —— 端上才说得出「最近的也有 80 公里」。
