...
public interface SplitGateway {
     ...
     * 这个实现<b>会不会有回执回来</b>。
     *
     * <p>为什么要有这一位：分账对账轴的覆盖范围说明里写着
     * 「分账网关目前是桩实现，所以每一笔已发出的单都不会有回执，
     * 这条轴报出来的量反映的是这件事，不是通道出了问题」——
     * <b>那句话是写死在字符串里的</b>。接了真通道之后它会变成假话，
     * 而没有任何东西会提醒：页面照常显示，读的人照常相信。
     *
     * <p>所以这句话改成从这里算出来。真实现不覆盖它，说明会自动消失。
     */
    default boolean deliversConfirmation() {
        return true;
    }
