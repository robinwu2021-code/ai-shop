...
import ai.neargo.shop.merchant.service.impl.StoreLinkServiceImpl;
...
class StoreLinkServiceTest {
    ...
    void returnsNullWhenBaseUrlNotConfigured() {
        for (String unset : new String[]{"", "   ", null}) {
            StoreLinkService svc = new StoreLinkServiceImpl(unset);
    ...
    void buildsStoreAndGoodsLinks() {
        StoreLinkService svc = new StoreLinkServiceImpl("https://m.neargo.ai");
    ...
    void trimsTrailingSlashes() {
        assertThat(new StoreLinkServiceImpl("https://m.neargo.ai///").linkOf(CODE, null))
