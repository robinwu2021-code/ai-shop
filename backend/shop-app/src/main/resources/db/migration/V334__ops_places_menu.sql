-- 运营端「固定地址库」菜单落库（TDD-C端收货地址-录入与定位重排 · 批次 G）。
--
-- 菜单不是 nav.ts 说了算：标签、分组、可见性都来自 sys_function_point / sys_role_point。
-- 只改 nav.ts 的话，本地 mock 下看得到，**接上真实后端就没有**。
--
-- 这一屏是**唯一能提前发现「地图快不行了」的地方**：熔断与额度是后端进程内的
-- 状态，买家那边只看得到「地名标没标陈旧」。没有这一屏，端点做了等于把信号扔了。
--
-- sort 取 80：70 是上一版的「位置分布」，同样插在末尾，既有行一行不动。
--
...
SELECT 'OPS_COMMUNITY__TAB_PLACES', 'OPS_COMMUNITY', '固定地址库', '社区网格',
       '/communities?tab=places', 'community:community:read', 'community:community:read',
