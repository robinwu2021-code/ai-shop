...
(
    ...
    PRIMARY KEY (id),
    UNIQUE KEY uk_mch_session_token (token_hash),
    KEY idx_mch_session_user (user_no, revoked_at),
    KEY idx_mch_session_expires (expires_at),
    KEY idx_mch_session_revoked (revoked_at)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='B端会话。只存令牌与主体，身份由 mch_account 现读';
...
(
    ...
    PRIMARY KEY (id),
    KEY idx_mch_login_user (user_no, at),
    KEY idx_mch_login_at (at)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='B端登录审计。保留 180 天';
