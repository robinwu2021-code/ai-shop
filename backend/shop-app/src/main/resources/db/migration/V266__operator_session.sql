...
(
    ...
    PRIMARY KEY (id),
    UNIQUE KEY uk_ops_session_token (token_hash),
    KEY idx_ops_session_user (user_no, revoked_at),
    KEY idx_ops_session_expires (expires_at),
    KEY idx_ops_session_revoked (revoked_at)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='运营端会话。roles/perms/scope 全部现读现算，不进会话';
...
(
    ...
    PRIMARY KEY (id),
    KEY idx_ops_login_user (user_no, at),
    KEY idx_ops_login_at (at)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='运营端登录审计。保留 730 天，运营操作要能追溯两年';
