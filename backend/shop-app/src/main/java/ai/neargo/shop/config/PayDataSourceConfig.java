 ...
 * {@code MybatisPlusConfig} 那个全局扫描把 {@code ai.neargo.shop.pay} 排除掉。
 * <b>少任何一头，结算的查询都会打回平台的工厂上</b> —— 而那时事务隔离静默失效，
 * 表现与今天一模一样（因为库是同一个），只有拆库那天才会炸。
 *
 * <p>{@code CrossDomainTxConventionTest} 盯着这件事：settle 里的
 * {@code @Transactional} 必须点名 {@code payTxManager}，漏一个就红。
 */
@Configuration
@MapperScan(basePackages = "ai.neargo.shop.pay.mapper",
        sqlSessionFactoryRef = "paySqlSessionFactory")
public class PayDataSourceConfig {
     ...
     */
    @Bean
    PlatformTransactionManager payTxManager(
            @Qualifier("payDataSource") DataSource ds) {
     ...
     */
    @Bean
    SqlSessionFactory paySqlSessionFactory(@Qualifier("payDataSource") DataSource ds,
