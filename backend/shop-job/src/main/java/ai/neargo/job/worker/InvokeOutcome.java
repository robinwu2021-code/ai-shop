 ...
 */
record InvokeOutcome(JobStatus status, String detail, String error, Integer httpStatus) {

    static InvokeOutcome of(JobStatus status, String detail, String error, Integer httpStatus) {
        return new InvokeOutcome(status, detail, error, httpStatus);
    }

    static InvokeOutcome unreachable(String error) {
        return new InvokeOutcome(JobStatus.UNREACHABLE, "调不通业务系统", error, null);
    }

    static InvokeOutcome timeout(int timeoutSec) {
