 ...
 * 商家支付进件 —— **每通道一条**。
 *
 * <p><b>为什么 subMchid 不放在 {@code usr_merchant} 上</b>：商家可能同时进件微信与支付宝，
 * 两边的商户号、进件状态、授权能力各不相同，<b>甚至主体类型都可能不同</b>
 * （微信进小微、支付宝进个体户）。一列表达不了。
 *
 * <p>这张表与 {@code usr_merchant_apply}（平台入驻审核）是两件事：
 * 平台审核通过 = 能上架卖货；支付进件通过 = 能收钱。合成一个状态就无法表达
 * 「审核过了但收不了钱」。
 */
@Getter
@Setter
@TableName("usr_merchant_payment")
public class UsrMerchantPayment extends BaseEntity {
