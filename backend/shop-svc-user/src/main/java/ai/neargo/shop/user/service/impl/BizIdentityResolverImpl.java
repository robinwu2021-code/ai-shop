...
public class BizIdentityResolverImpl implements BizIdentityResolver {
    ...
    public BizIdentityResolverImpl(MchEntityMapper merchantMapper, PickupPointMapper pickupMapper,
    ...
    public BizContext resolve(String userNo) {
        ...
        var memberships = staffMapper.selectList(
                Wrappers.<ai.neargo.shop.user.merchant.entity.MchAccount>lambdaQuery()
                        /*
                         * **两条登录路径解析到同一条员工记录**：
                         *   小程序 → C 端账号（user_no）
                         ...
                         * 而他看到的只是「打不开」。
                         */
                        .and(q -> q.eq(ai.neargo.shop.user.merchant.entity.MchAccount::getUserNo, userNo)
                                .or().eq(ai.neargo.shop.user.merchant.entity.MchAccount::getMchAccountNo, userNo))
                        .eq(ai.neargo.shop.user.merchant.entity.MchAccount::getStatus,
                                ai.neargo.shop.user.merchant.entity.MchAccount.ACTIVE)
                        .orderByDesc(ai.neargo.shop.user.merchant.entity.MchAccount::getIsPrimary)
                        .orderByAsc(ai.neargo.shop.user.merchant.entity.MchAccount::getId));
        ...
        Set<String> pickupNos = pickupMapper.selectList(Wrappers.<CmtPickupPoint>lambdaQuery()
                        .eq(CmtPickupPoint::getOwnerRef, merchant.getEntityNo())
