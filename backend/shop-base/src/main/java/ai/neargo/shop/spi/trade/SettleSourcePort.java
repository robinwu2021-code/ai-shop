...
public interface SettleSourcePort {
    ...
    /**
     * 这批<b>订单</b>里不是已支付状态的（不变式 I8 的右边）。
     *
     * <p>与上面那个是同一件事的两个粒度：{@link #notPaidAmong} 看子单，
     * 给的是 I2「每张结算单都对得上一个已支付子单吗」；
     * 这个看主单，给的是 I8「支付域收到的每笔钱，订单都转 PAID 了吗」。
     *
     * <p><b>粒度不能混用。</b>一笔支付对应的是主单，而结算是按子单做的 ——
     * 拿子单的方法去比主单，查不到的会被当成「不是已支付」，
     * 于是 I8 每轮都会去补一批本来就没问题的订单。
     * 补的动作幂等，所以不会出事，但<b>「补了几个」这个数会永远等于扫描数</b>，
     * 而那个数正是判断 I8 有没有在真的发挥作用的唯一依据。
     *
     * <p>同样返回「异常的那些」而不是「正常的那些」，理由同上。
     * 空集合进来要返回空集合，不要当成「全都没付」。
     */
    List<String> notPaidOrders(java.util.Collection<String> orderNos);
