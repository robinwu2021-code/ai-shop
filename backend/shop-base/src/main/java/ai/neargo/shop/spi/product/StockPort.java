...
public interface StockPort {
    ...
    /**
     * 此刻<b>最多还能下单多少件</b> —— 给加购、改数量做前置校验。
     *
     * <h2>必须与 {@link #lock} 用同一套规则</h2>
     * 购物车校验只是「提前告诉你」，真正的闸门是下单时的 {@link #lock}（原子、按门店）。
     * 两处规则不一致的后果分两种，都不报错：
     * <ul>
     *   <li>这里比 lock 严 → <b>误拒</b>：预售商品、别家门店有货的商品加不进购物车，
     *       而它们其实下得了单；</li>
     *   <li>这里比 lock 松 → 加购成功、结账时才说不足，等于这道校验没做。</li>
     * </ul>
     * 所以规则在实现里只写一处，照着 lock 的分支走：
     * <ul>
     *   <li>现货与预售<b>二选一、不叠加</b>（lock 先整单试现货，不够再整单试预售）；</li>
     *   <li>分店库存的 SKU <b>取各店里最能卖的那一家</b> —— 加购时还不知道由哪家店履约，
     *       取最大值是为了不误拒；它不走预售（与 lock 一致）。</li>
     * </ul>
     *
     * <h2>它不是锁</h2>
     * 返回的是一个<b>瞬时</b>的数，不占库存。两个人同时看到「还剩 1 件」都能加进购物车，
     * 这是有意的：购物车锁库存会让人把货囤在车里不买，别人买不到。
     * 谁先下单谁拿到，由 lock 的原子更新裁决。
     *
     * <p>默认实现返回 {@link Integer#MAX_VALUE}，意思是「这个真相源算不出来，不在加购时拦」——
     * 放行而不是拒绝：加购时误拒的代价（顾客以为没货走了）比晚一步在结账时拦住更大。
     *
     * @return 可下单的最大件数，不小于 0；查不到这个 SKU 时返回 0
     */
    default int sellable(String skuNo) {
