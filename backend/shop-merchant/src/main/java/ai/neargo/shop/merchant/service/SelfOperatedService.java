...
public interface SelfOperatedService {
    ...
    /**
     * 在运营端给一个主体再开一家门店。<b>两支，只差订阅额度</b>。
     *
     * <p>第三方商家自己在 B 端开店（{@code POST /biz/store/create}），而平台自己的店
     * 没有「商家」去点那个按钮 —— 与建主体是同一个形状的缺口，所以开在运营端。
     * 三期又加了第二种处境：BD 站在店里替老板配第二家店。
     *
     * <table>
     *   <tr><th></th><th>{@code self_operated=1}</th><th>第三方</th></tr>
     *   <tr><td>订阅额度</td><td>不吃</td><td><b>照吃</b></td></tr>
     * </table>
     *
     * <p><b>二期这里原本是「非自营一律拒」</b>，理由是「运营能绕过商家吃掉他买的额度」。
     * 三期把那个理由交还给额度闸本身：挡住超额的应该是额度，而不是「不许代开」——
     * 否则代开这件事整个做不了。超额时抛的是额度错，不是 {@code BAD_REQUEST}。
     *
     * <p><b>自营不走订阅额度</b>：额度是卖给商家的商品，平台自己的店不该被自己的定价限制。
