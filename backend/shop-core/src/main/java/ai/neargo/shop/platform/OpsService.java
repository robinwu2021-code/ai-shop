...
public interface OpsService {
     ...
     * {@code agreed_at}。
     *
     * <p><b>只能由本人调</b>，参数是登录身份，不接受传入「替谁勾」——
     * 运营代填的单子上这一栏一律留空，而能把它填上的只有本人这一条路。
     * 开一个「替谁勾」的参数，等于把刚刚拦住的那件事从后门放进来。
     *
     * <p><b>幂等</b>：已经勾过就原样返回那个时刻，不刷新成现在。
     * 刷新会让「他什么时候同意的」这个问题每次打开 App 都换一个答案，
     * 而这正是这一列唯一的用途。
     *
     * @return 同意的时刻（毫秒）。没有申请单时返回 0 —— 没申请过不是异常
     */
    long acceptAgreement(String userNo);
