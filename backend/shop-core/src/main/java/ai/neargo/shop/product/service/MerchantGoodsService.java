...
public interface MerchantGoodsService {
    ...
    /**
     * 把**所有有商品的主体**的社区池重建一遍。运营端一次性入口。
     *
     * <p><b>为什么需要它</b>：社区池是派生索引，而口径是会变的。
     * 2026-08-25 把可见性从「主体可达」改成「门店可达 × 门店选品」之后，
     * 存量池行的 {@code store_no} 是迁移回填的默认店 —— 新口径下只有
     * 「默认店确实服务那个社区」才碰巧正确。当时线上全部门店都是 ALL 范围，
     * 所以确实正确，但**依赖这个巧合不如显式重建一次**。
     *
     * <p>往后每次动口径都该跑一次；平时怀疑「商家说在售、买家搜不到」也可以跑。
     * 幂等，跑几次都一样。
     *
     * <p>范围是「有商品的主体」而不是全部主体：没有商品就没有池行可建。
     *
     * @return 重建了几件商品
     */
    int resyncAllCommunityPools();
