...
def main(quiet: bool = False) -> int:
    ...
    L += ["### 4.2 同一组端点被两个域的码分着守", "",
          ...
          "**这个判据不能当闸门** —— 下面四组里三组是刻意的，理由就写在代码注释里。",
          "列出来是为了让第四组显出来。", ""]
    L += ["| 路径段 | 分给了 | 结论 |", "|---|---|---|"]
    ...
    L += ["",
          "**进销存那条要单独说。** 域 18 在运营端没有自己的命名空间，7 个",
          "`/ops/inventory/*` 端点整体寄在别人名下：只读那 5 个挂 `product:sku:read`，",
          "而**发放与吊销开放对接凭证**这两个挂 `merchant:mode:update`。",
          "于是授予商品运营岗看 SKU 的权，就一并授出了全平台库存台账与对账；",
          "授予商家运营岗改经营模式的权，就一并授出了**给外部系统开 API 钥匙**的权力。",
          "**两处在权限配置界面上都看不出来** —— 界面上只有码，没有「这个码还管着什么」。", ""]
