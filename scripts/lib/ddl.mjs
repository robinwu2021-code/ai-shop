...
export function readSchema(root) {
  ...
  for (const m of sql.matchAll(
    ...
    // 表选项**允许跨行，但有上界**（`[^;]{0,600}`）。
    //
    // 上界防的不是负向字符组本身（它一遍扫到下一个 `;` 就停，不回溯），
    // 而是它与前面那个**惰性 body**（`[\s\S]*?`）的组合：收尾匹配不上时，
    // body 会向后扩一格再试一次，每扩一格都重新扫一遍尾巴 —— 几十个迁移
    // 拼成一条时间线之后就是 O(n²)，实测跑到分钟级不出结果，看着像卡死。
    // 有上界就没有这个空间。
    //
    // 600 是量出来的，不是拍的：全仓 148 处收尾，最长 210、90 分位 122、中位数 93。
    // 300 只剩 90 字符余量 —— 一条中文 COMMENT 加三十个字就越界，
    // 而 600/1200/4000 三档实测都是 1ms，抬高不要钱。
    // 越界也不是无声的：`tests/ddl-parsable.test.ts`（「声明过的表解析器都要读得到」）
    // 会点名报出来，那是这条上界的网。**没有那张网之前不要动这个数**。
    //
    // 完全不许跨行（`[^;\n]*`）也有代价，而且这个代价已经付过了：
    // V152 里两处写成
