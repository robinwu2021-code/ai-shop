...
BLOCK_NOTES = {
    ...
    ".sh-row--baseline": ("行与列", "两端对齐但按**基线**：左右字号不同时用它，两段字才坐在同一条线上", "字号一样就别加，center 更稳"),
    ".sh-row--top": ("行与列", "两端对齐但顶对齐：右边的值会折成两行时用", "单行值用 .sh-row--between"),
    ".sh-row--divided": ("行与列", "连排的行之间画一道发丝线（第一行不画）", "行与行之间要留白的用 .sh-cells"),
    ".sh-wrap": ("行与列", "会折行的一堆：标签组、筛选 chip 组、一排动作，缝 12rpx", "一行装得下的横排用 .sh-row"),
    ".sh-fill": ("行与列", "占满剩余宽度并允许被压缩（`flex:1;min-width:0`）", "定宽的那一侧别挂"),
    ".sh-center": ("行与列", "内容摆正中间的方块", "只要横向居中用 text-align"),
    ".sh-cells": ("容器", "一组设置项/入口挤在一块白底上，行间只留 2rpx 缝", "行与行要分开成卡的用 .sh-card"),
    ".sh-cell": ("容器", ".sh-cells 里的一行，内边距跟着 --sh-pad-page 走", "别单独用，它没有底色"),
    ".sh-notice": ("容器", "一段话压在语义色 tint 底上：主色档（说明、进行中）", "可点的用 .sh-btn，短标签用 .sh-chip"),
    ".sh-notice--warning": ("容器", "提示条的**提醒**档：要注意但还没出错", "已经出错用 --danger"),
    ".sh-notice--danger": ("容器", "提示条的**出错**档：驳回、失败、被停用", "只是提醒用 --warning"),
    ".sh-notice--success": ("容器", "提示条的**完成**档：通过、已结算", "进行中用主色档"),
    ".sh-notice--muted": ("容器", "提示条的**中性**档：没有语义，只是这段话与正文分开", "有语义就挑上面四档"),
    ".sh-searchbox": ("表单", "搜索框，以及「点一下跳到搜索页」的假输入框。胶囊、88rpx 高", "真要收字的用 .field__input"),
    ".sh-seg": ("行与列", "分段/筛选里的一格：弱色底、选中翻主色", "整块可选的卡用 sh-option"),
    ".sh-seg--on": ("行与列", "分段格的选中态", "—"),
    ".sh-seg--off": ("行与列", "分段格的不可选态：压暗且不响应", "—"),
    ".sh-seg--fill": ("行与列", "分段格平分整行宽度", "个数不定时别用，会挤扁"),
    ".sh-hint": ("排版", "紧跟在某一行下面的一句小字说明（block + 上缝 8rpx）", "整段独立的说明用 .sh-muted"),
    ".sh-hit": ("行与列", "把点按面积撑到 88rpx 以上，而不改变它看上去的大小", "本来就够大的别挂"),
    ".sh-scrollx": ("行与列", "横向滚动一排（隐藏滚动条）", "会折行的用 .sh-wrap"),
    ".sh-badge-count": ("标签", "红底白字的计数小圆点：未读数、购物车件数", "不带数字的红点另说"),
    ".sh-chip--icon": ("标签", "里面带图标的 chip（选中的勾、可删的 ✕）", "纯文字用 .sh-chip"),
    ".sh-chip--solid": ("标签", "实心主色 chip：当前选中的那一个", "未选中用 .sh-chip"),
    ".field__input": ("表单", "单行输入框：88rpx 高、弱色底、正文字号", "多行用 .field__area"),
    ".field__area": ("表单", "多行输入框。**高度由页面给**（min-height），盒子归它", "单行用 .field__input"),
    ".field__label": ("表单", "输入框上方的名目", "—"),
    ".field__head": ("表单", "名目那一行（名目 + 右侧的计数或动作）", "—"),
