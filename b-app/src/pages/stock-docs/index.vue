 ...
 *   比没有按钮更糟；
 * - **盘点单**：它的作废在盘点页自己那儿（2026-09-17 补的端点）。
 *   这里给不了 —— 盘点行与调拨行都是**点了就跳走**，压根不展开，
 *   下面那个作废按钮只对入库/出库两类可见。
 *
 * ⚠️ **于是 `TRANSFER` 那半条是死的**：`canVoid` 认它、`voidDoc` 也认它，
 *   可 `open()` 一进来就 `navigateTo` 走了，展开区永远不渲染。
 *   INV-W15 写的「两处入口」今天只剩详情页那一处。**没有在这次一并改** ——
 *   要么把调拨行改成可展开、要么把这半条删掉，是另一个决定。
 *
