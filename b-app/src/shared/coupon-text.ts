...
/** 类型键：次卡按次数判（任何面额方式，能核销多次就是次卡），其余按优惠方式 */
export function couponKind(c: Pick<MerchantCoupon, "benefitMode" | "timesTotal">): string {
  if ((c.timesTotal ?? 1) > 1) return "TIMES";
  return c.benefitMode || "CASH";
}

/** 「现金 ¥5」「9 折 · 封顶 ¥20」「次卡 · 5 次」 */
export function couponRule(t: T, c: MerchantCoupon): string {
  const kind = couponKind(c);
  if (kind === "TIMES") return t("couponText.ruleTimes", { n: c.timesTotal });
  if (kind === "PERCENT") {
    return t("couponText.rulePercent", { z: (c.benefitValue / 1000).toFixed(1).replace(/\.0$/, ""),
      cap: money(c.benefitCapMinor ?? 0) });
  }
  if (kind === "GIFT") return t("couponText.ruleGift");
