...
export interface MerchantApi {
   ...
   * <b>本人补勾《商家服务协议》</b>（三期）。
   *
   * 运营代商家进件时不能替他勾协议，那张单落库时这一栏是空的；
   * 而商户自己走的入驻表单里今天也没有这一勾 —— 登录页那一勾一路传到
   * 后端 `LoginCommand.agreed` 就断了，实现里一次都没引用过。
   * 所以这条接口对两条路都有意义：它是这套系统第一次真的记录协议同意。
   *
   * **只能勾自己的**：后端取登录身份，不接受「替谁勾」的参数。
   *
   * @returns 同意的时刻（毫秒）；没有申请单时 0。已经勾过返回原来那个时刻（幂等）
   */
  mAcceptAgreement(): Promise<number>;
  /**
   * 平台主数据：行业 / 主体类型 / 支付通道。
   *
