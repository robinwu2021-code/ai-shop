...
describe("stock-entries", () => {
  it("底下那条只留进货和报损", () => {
    const { primary, more } = stockEntries({ can: boss, multiStore: false, usableLocations: 3 });
  ...
  it("「更多」里是六条，按多久用一次排", () => {
    const { more } = stockEntries({ can: boss, multiStore: false, usableLocations: 3 });
    expect(keys(more)).toEqual(["check", "transfer", "docs", "report", "locations", "suppliers"]);
  });

  it("跨店只给多门店商家，且不在那六条里", () => {
    const one = stockEntries({ can: boss, multiStore: false, usableLocations: 3 });
    expect(one.cross).toBeNull();

    const many = stockEntries({ can: boss, multiStore: true, usableLocations: 3 });
  ...
  it("放货的地方不足两个时调拨不可点，并给出去处", () => {
    const { more } = stockEntries({ can: boss, multiStore: false, usableLocations: 1 });
  ...
  it("没有库位权限的人不给去处，但仍要看到原因", () => {
    const { more } = stockEntries({ can: clerk, multiStore: false, usableLocations: 1 });
    const transfer = more.find((e) => e.key === "transfer")!;
    expect(transfer.blocked!.reasonKey).toBe("stock.entryBlocked.transfer");
  ...
  it("还没取到库位数时不灰掉 —— 不知道不等于不足两个", () => {
    const { more } = stockEntries({ can: boss, multiStore: false, usableLocations: null });
    expect(more.find((e) => e.key === "transfer")!.blocked).toBeUndefined();
  });

  it("每条按它自己那一页的权限判", () => {
    const { more, primary } = stockEntries({ can: clerk, multiStore: true, usableLocations: 3 });
  ...
  it("一个码都没有的人：菜单是空的，不是一排点不动的名字", () => {
    const { primary, more } = stockEntries({
      can: () => false, multiStore: true, usableLocations: 3,
    });
