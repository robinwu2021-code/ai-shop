...
describe("选点页：把「选」提为主路", () => {
  ...
  it("★★★ 没坐标的社区不许出现在列表里，**整段的显隐也看过滤之后的条数**", () => {
    ...
    // 钉的是**用哪个数**，不是那一行的完整写法：2026-09-09 这一段加了
 ...
 */
/*
 * **粘贴识别已经去掉**（2026-09-18）。原来这儿有一个 describe，两条守卫：
 * 「只填空着的格子」与「认不出来要说一声」。
 *
 ...
 *
 * ⚠️ 删用例时**整个 describe 一起删**：只掏空里面、留一个空壳，
 * vitest 会报 `No test found in suite` —— 而它不显示成 `×`，
 * 只在摘要上方单出一行 FAIL，「174 passed」照样印着。
 */
...
describe("定位只匹配收货地址", () => {
  ...
  it("★★★ 「当前位置」给过的那个，不在「附近」里再给一遍", () => {
    /*
     * 实测（龙华导入 2783 个小区之后）：站在景龙新邨东区，「当前位置」写着它，
     * 「附近」第一条又是它 —— 而且距离是 0、那一行连距离都不显示，
     * 看起来像另一个同名的地方。同一个东西在一屏上出现两次，
     * 读的人要先判断「这俩是不是一回事」。
     */
    expect(pickPage, "没有去重").toMatch(/filter\(\(c\) => c\.name !== hereName\.value\)/);
  });
