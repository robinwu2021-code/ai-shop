...
// 积分账户。C 端（我的积分）与 B 端（商家收到的积分）共用这一页，靠 `side` 参数切换。
//
// 两侧的账本结构完全一样（余额 + 流水 + 变动后余额），差别只在语义：
//   C 端：EARN 消费获得 / USE 下单抵扣 / REFUND 退款返还
//   B 端：RECEIVE 收到用户抵扣的积分 / SETTLE 平台已兑付
// 所以不做两个页面。
  ...
  <sh-scaffold :title-key="side === 'merchant' ? 'points.merchantTitle' : 'points.title'">
    ...
    <view class="sh-card hero">
      <text class="hero__label">
        {{ side === "merchant" ? $t("points.received") : $t("points.balance") }}
      </text>
      ...
      <view class="sums">
        <view class="sum">
          ...
          <text class="sum__k">
            {{ side === "merchant" ? $t("points.totalReceived") : $t("points.totalEarned") }}
          </text>
        </view>
        <view class="sum">
          <text class="sum__v sh-num">{{ account?.totalUsed ?? 0 }}</text>
          <text class="sum__k">
            {{ side === "merchant" ? $t("points.totalSettled") : $t("points.totalUsed") }}
          </text>
        </view>
      </view>
    </view>
    ...
    <view class="sh-card block rules">
      ...
      <text class="rules__t">
        {{ $t("points.ruleExpire", { n: POINTS.validDays }) }}
      </text>
