...
/**
 * 省 / 市 / 区三级选择器 —— **收货地址**用的那个。
 *
 * ⚠️ **2026-09-08 从 `biz-region-picker` 改名**：b 端有一个同名件，做的是
 * 完全不同的事（经营范围，市 › 区 › 街道 › 小区/村 四级 + 搜索 + 提报，722 行逻辑）。
 * 两个不相干的东西同名，谁读代码、谁 grep、谁看组件清单都会把它们当成一个 ——
 * 组件清单里 `biz-region-picker` 出现两次、行数差 10 倍，混淆当场就发生了。
 * 名字要说清它挑的是什么。
 *
 *
