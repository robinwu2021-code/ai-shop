...
#
# 脚本传的是开发版，而**体验版会跟到这一版**（2026-09-06 确认，后台不用再点）——
# 也就是说跑完这条命令，拿体验版码的人看到的就是新的。
# 提审与发布仍然留给人：那两步不可逆，且要选类目、传功能页面截图。
set -euo pipefail
...
echo
echo "传完了 —— **体验版已经是这一版**（后台不用再点，2026-09-06 确认）。"
echo
echo "还要人点的只剩上线那两步（脚本做不了：miniprogram-ci 2.1.31 只导出"
echo "upload / preview，微信没给自有小程序开提审与发布的接口）："
echo "  1. 提交审核（要选类目、传功能页面截图）"
echo "  2. 审核通过后点「发布」"
