...
export function RoutingTab({ c, canEdit }: { c: MessageCopy; canEdit: boolean }) {
   ...
   */
  const groups = new Map<string, { scene: string; audience: string; cells: SceneChannelCell[] }>();
  for (const r of rows) {
        ...
        <table className="w-full min-w-[44rem] text-[13px]">
          <thead>
            <tr className="border-b bg-muted/40">
              <th className="px-3 py-2 text-start font-medium">{c.rtColScene}</th>
              <th className="px-3 py-2 text-start font-medium">{c.rtColAudience}</th>
              {CHANNELS.map((ch) => (
                <th key={ch} className="px-3 py-2 text-start font-medium">{c[`ch_${ch}` as keyof MessageCopy]}</th>
              ))}
            </tr>
          </thead>
          <tbody className="divide-y">
            {[...groups.values()].map((g) => (
              <tr key={`${g.scene}|${g.audience}`}>
                <td className="px-3 py-2">{c[`scene_${g.scene}` as keyof MessageCopy] ?? g.scene}</td>
                ...
                {CHANNELS.map((ch) => {
                   ...
                   * 都画成一个关着的开关就把它们混成了一件事 —— 而前者点不动、
                   * 后者一点就生效。没有的格子留白。
                   */
                  if (!cell) return <td key={ch} className="px-3 py-2 text-muted-foreground/50">—</td>;
                  return (
