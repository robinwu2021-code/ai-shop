...
export interface MerchantApply {
  ...
  auditedAt?: number;
  /**
   * <b>这张单是运营代填的</b>（三期）。审核台要看到它 ——
   * 资料不是商户自己录的，核验那一步该更仔细。
   *
   * <p>⚠️ 看到这个标记<b>不等于你审得动</b>：后端还拦「代填的人审自己填的那一张」
   * （403）。BD 同时持有审核码与代填码，光靠角色配置挡不住，所以分离钉在数据上。
   */
  onBehalf?: boolean;
  /**
   * 商户本人同意《商家服务协议》的时刻（毫秒）；<b>0 / 空 = 尚未同意</b>。
   *
   * <p>代填的单子这一栏一律为空 —— 运营不能替商户勾，等他首次登录自己补。
   * <b>存量单子同样是空的</b>（协议勾选此前从没落过库），所以「空」不等于
   * 「他拒绝了」，也不等于「这是代填单」：要分开看 {@link onBehalf}。
   */
  agreedAt?: number;
}
