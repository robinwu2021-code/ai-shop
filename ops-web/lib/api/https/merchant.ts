...
export const merchantHttp: MerchantApi = {
  ...
  merchantFulfillment: (merchantNo) => client.get(`/ops/merchants/${merchantNo}/fulfillment`),
  lockChannel: (storeNo, channel, reason) =>
    client.post(`/ops/stores/${storeNo}/channels/${channel}/lock`, { reason }),
  unlockChannel: (storeNo, channel) =>
    client.post(`/ops/stores/${storeNo}/channels/${channel}/unlock`),
  ...
  applyOnBehalf: (v) => client.post("/ops/merchants/apply-on-behalf", v),
  ...
  nudgeMerchant: ({ entityNo, ...body }) =>
    client.post(`/ops/merchant/${encodeURIComponent(entityNo)}/nudge`, body),
