...
function implementedEndpoints(): Set<string> {
  ...
  for (const f of readdirSync(dir).filter((x) => x.endsWith(".ts"))) {
    ...
    // 也认**包装调用**：`toStaff(await client.post(...))` —— 需要把后端形状
    // 映射成前端类型的端点都长这样，而漏认它的表现与嵌套泛型那次一样：
    // 报的是「这个方法没有 client 调用」，而它明明写着。
    // 泛型参数用 `[^(\n]*` 而不是 `[^>]*`：**嵌套泛型**（client.get<Foo<Bar>>）
    // 与**带分号的行内对象泛型**（client.get<{ a: string; b: number }>）都要认。
    // 认不出的失败样子一模一样：400 字窗口一路吃到下一个方法，
    // 于是报的是**下一个方法**没有 client 调用，而那个方法明明写着 ——
    // 一条把人指向无关代码的报错，比不报还费时间。这个坑今晚踩了两次。
