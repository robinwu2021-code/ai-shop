...
describe("页面层同样受约束（基线 0，不留额度）", () => {
  ...
  it("components/ 绕开字阶的存量只准变短（基线 1，来源见注释）", () => {
    /*
    ...
    const BASELINE: Record<string, number> = {
       ...
       * **只剩一处，而且它不是「还没轮到」。**
       *
       * 表头写的是 `[&_th]:text-xs`。换成 `[&_th]:txt-caption` 会**一条 CSS 规则都不生成**
       * —— Tailwind 只给自己的工具类做任意变体，`txt-caption` 是手写类，它不认。
       * 类名照样在 DOM 上，样式不存在，表头静默退回继承的 14px
       * （2026-09-09 换过一次，当场量到 12→14px，已撤回）。
       * 要给它上档，得把表头排版写进 globals.css 的 `@layer components`，那是另一件事。
       */
      "ui/table.tsx": 1,
    };
