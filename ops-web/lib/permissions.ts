...
export const BACKEND_ROLE_PERMS: Record<string, string[]> = {
  SUPER_ADMIN: ["*"],
  MERCHANT_BD: ["aftersale:refund:read", "aftersale:ticket:read", "community:community:read", "dashboard:overview:read", "group:demand:assign", "group:demand:read", "merchant:apply:audit", "merchant:category:grant", "merchant:category:read", "merchant:merchant:ban", "merchant:merchant:read", "merchant:mode:read", "merchant:mode:update", "merchant:fulfillment:update", "growth:attribution:read", "merchant:verify:grant", "order:order:read", "store:page:audit"],
  // member:*（P8）：**看名单与人档，不含看完整号与合并** ——
  // 后四位够他判断「这个人是谁家的会员」；还原成真实号码是申诉处置的事
  PRODUCT_OPS: ["aftersale:refund:read", "aftersale:ticket:read", "community:community:read", "dashboard:overview:read", "group:campaign:audit", "group:campaign:read", "marketing:campaign:read", "marketing:campaign:update", "marketing:coupon:issue", "marketing:coupon:read", "marketing:coupon:update", "member:member:read", "member:person:read", "order:order:read", "product:category:read", "product:category:update", "product:sku:audit", "product:sku:read", "product:spec:read", "product:spec:update", "product:std:read", "product:std:update", "product:topic:read", "product:topic:update"],
