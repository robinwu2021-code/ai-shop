...
| 字段 | 类型 | 必填 | 说明 |
|---|---|:---:|---|
| `slotNo` | `string` | 是 | 时段号。**下单占的是它** |
| `storeNo` | `string` | 是 | 门店号 |
| `startAt` | `number` | 是 | 开始时刻（毫秒） |
| `endAt` | `number` | 是 | 结束时刻（毫秒） |
| `capacity` | `number` | 是 | 这一档能约几个人 |
| `booked` | `number` | 是 | 已约人数 |
| `remaining` | `number` | 是 | 还能核几次 |
...
| `remaining` | `number` | 是 | 还能核几次 |
| `usedUp` | `boolean` | 是 | 次数用尽 |
| `duplicated` | `boolean` | 是 | 为真 = 店员**连点了两下**（3 秒窗口内），不是第二次核销 —— 报错会让他以为没成功，于是再按一次 |
...
| 字段 | 类型 | 必填 | 说明 |
|---|---|:---:|---|
| `entity` | [`Entity`](#entity) | 是 | 所属主体（一张证照） |
...
| 字段 | 类型 | 必填 | 说明 |
|---|---|:---:|---|
| `recommend` | `string` | 是 | 带楼盘/门牌的人话版地址。**端上填这个** |
| `address` | `string` | 是 | 地址 |
...
#### POST `/biz/group/{groupNo}/dissolve`
...
请求体：[`DissolveGroupReq`](#dissolvegroupreq)

| 字段 | 类型 | 必填 | 说明 |
|---|---|:---:|---|
| `reason` | `string` | 否 | 散团原因，写进参团买家的退款记录；可空 |
...
类型：[`GroupPickupOption`](#grouppickupoption)\[\]
...
| `activityNo` | `string` | 否 | 页面上选的拼团活动；与这件货此刻所在的活动对不上时后端拒 |
| `pickupNo` | `string` | 否 | 成团范围（自提点）；不传 = 不限点 |
...
| `status` | [`TransferStatus`](#transferstatus) | 是 | 见  {@link  TransferStatus } |
...
| `enrollable` | `number` | 是 | 还能报名的平台活动数 |
...
| `reachable` | `number` | 是 | 其中**能真正收到东西**的有多少（线索会员与已退订的人进不了受众）。只显示 count 的话，商家在人群页看到 120、发放页发出 96，会以为发漏了 |
| `pushable` | `number` | 是 | 能收到的人里，还会收到手机推送的（有推送设备）。其余只进小程序的消息列表 |
...
| 字段 | 类型 | 必填 | 说明 |
|---|---|:---:|---|
| `taskNo` | `string` | 是 | 这次群发的任务号 |
| `sent` | `number` | 是 | 实际发出多少条 |
| `pushed` | `number` | 是 | 其中推送到手机的人数；其余只进了小程序的消息列表 |
| `skipped` | `number` | 是 | 跳过多少人 |
| `skips` | `object`（见下）\[\] | 是 | 发不出去的人按原因分类。**必须显示** —— 商家选了 30 个人实发 8 个，只说「发送成功」他会以为 30 个都收到了 |
...
| 字段 | 类型 | 必填 | 说明 |
|---|---|:---:|---|
| `affectedMembers` | `number` | 是 | 会被改到的会员数 |
| `bothTagged` | `number` | 是 | 两个标签都有的人。合并后只保留一条 |
| `referencedActivities` | `number` | 是 | 引用了这个标签的活动数。**合并前要看** —— 合并不可逆，而活动的受众条件会跟着变 |
...
| `d90OrderCount` | `number` | 是 | 近 90 天下单数。**分层判据用它而不是累计** —— 三年前买过十次的人今天是沉睡客 |
| `lastOrderAt` | `number,null` | 否 | 上次下单时刻。空 = 从没下过单（线索会员） |
| `daysSinceLast` | `number,null` | 否 | 距上次下单多少天。按 lastOrderAt 与今天实时算 |
...
| 字段 | 类型 | 必填 | 说明 |
|---|---|:---:|---|
| `newCount` | `number` | 是 | 新客数 |
| `regularCount` | `number` | 是 | 回头客数 |
| `loyalCount` | `number` | 是 | 忠实客数 |
| `sleepingCount` | `number` | 是 | 沉睡客数 |
...
| `agreementPending` | `boolean` | 否 | <b>《商家服务协议》还没人勾</b>（三期）。 <p>只在**代填的单子**上为真 —— 运营代商家进件时不能替商户勾协议。 <p>⚠️ **不是「agreed_at 为空」**：存量单子这一列同样全是空的 （协议勾选此前从没落过库），那样判会让全体存量商家明天一早都看到 一条「你还没同意协议」，而他们当初确实勾过、只是没人记下来。 一条对所有人恒亮的提示等于没有提示。 <p>当前**只提示不拦截**：今天没有提现可挂，而拦上架或拦收款会打断 一家已经审核通过的店的生意 —— 协议没勾是平台流程造成的（运营代填）。 |
...
| 字段 | 类型 | 必填 | 说明 |
|---|---|:---:|---|
| `pickupNo` | `string` | 是 | 取货点号 |
| `name` | `string` | 是 | 名称 |
| `address` | `string,null` | 否 | 地址 |
| `type` | [`PickupPointType`](#pickuppointtype) | 是 | 类型 |
| `status` | `string` | 是 | ACTIVE / PENDING / REJECTED …；本店自建的 PENDING 点可引用，别家的不行 |
| `communityNo` | `string` | 是 | 所在社区号 |
| `communityName` | `string,null` | 否 | 所在社区名 |
| `ownerStoreNo` | `string,null` | 否 | STORE 点的承接门店；= 本店即「我自建的」 |
| `rejectReason` | `string,null` | 否 | 被驳回的原因。**要原样回商家** —— 不写等于让人猜 |
...
类型：[`PlatformActivity`](#platformactivity)\[\]


#### GET `/biz/platform-activity/{activityNo}`
...
| 字段 | 类型 | 必填 | 说明 |
|---|---|:---:|---|
| `activityNo` | `string` | 是 | 活动单号 |
| `name` | `string` | 是 | 名称 |
| `triggerType` | `string` | 是 | 触发：NONE 立减 / AMOUNT 满额 / QTY 满件 |
| `triggerAmountMinor` | `number,null` | 否 | 满多少（分） |
| `triggerQty` | `number,null` | 否 | 满几件 |
| `benefitType` | `string` | 是 | 优惠：现只有 CUT（减钱） |
| `benefitAmountMinor` | `number,null` | 否 | 减多少（分） |
| `startAt` | `number,null` | 否 | 活动开始 |
| `endAt` | `number,null` | 否 | 活动结束 |
| `enrollDeadline` | `number,null` | 否 | 报名截止 |
| `platformShareBp` | `number` | 是 | 平台出资万分比：10000 全额 / 5000 一半 / 0 不出 |
| `budgetMinor` | `number,null` | 否 | 平台预算（分） |
| `reservedMinor` | `number` | 是 | 已通过的报名占掉的预算 |
| `perOrderPlatformMinor` | `number` | 是 | 每单平台最多补贴 |
| `perOrderMerchantMinor` | `number` | 是 | 每单商家最多承担 |
| `enrollRule` | [`PlatformEnrollRule`](#platformenrollrule) | 是 | 报名门槛 |
| `status` | `string` | 是 | DRAFT / RUNNING / ENDED |
| `submitted` | `number` | 是 | 待审报名数 |
| `approved` | `number` | 是 | 已通过报名数 |
| `rejected` | `number` | 是 | 已驳回报名数 |
| `mine` | [`PlatformEnrollment`](#platformenrollment) \| `null` | 否 | 商家端：我的报名；没报过为空 |


#### POST `/biz/platform-activity/{activityNo}/enrollment`
...
请求体：[`EnrollReq`](#enrollreq)

| 字段 | 类型 | 必填 | 说明 |
|---|---|:---:|---|
| `goodsNos` | `string`\[\] | 是 | 报名的货：必须是自己的、在售的 |
| `quota` | `number` | 是 | 报多少份 |

**出参**（`data`）

类型：[`PlatformEnrollment`](#platformenrollment)

| 字段 | 类型 | 必填 | 说明 |
|---|---|:---:|---|
| `enrollmentNo` | `string` | 是 | 报名单号 |
| `activityNo` | `string` | 是 | 平台活动 |
| `entityNo` | `string` | 是 | 报名的商家 |
| `merchantName` | `string` | 是 | 商家名 |
| `goodsNos` | `string`\[\] | 是 | 报名的货 |
| `quota` | `number` | 是 | 报的份数 |
| `quotaUsed` | `number` | 是 | 已用份数 |
| `platformMaxMinor` | `number` | 是 | 最多平台补贴（最小货币单位）= 份数 × 每单平台补贴 |
| `merchantMaxMinor` | `number` | 是 | 最多商家承担 = 份数 × 每单商家承担 |
| `rating` | `number` | 是 | 商家评分（审核时看） |
| `status` | [`EnrollmentStatus`](#enrollmentstatus) | 是 | 状态 |
| `rejectReason` | `string,null` | 否 | 驳回理由 |
| `reviewedAt` | `number,null` | 否 | 审核时间 |
| `createdAt` | `number` | 是 | 提交时间 |


#### POST `/biz/platform-activity/{activityNo}/withdraw`
...
| 字段 | 类型 | 必填 | 说明 |
|---|---|:---:|---|
| `items` | [`Qualification`](#qualification)\[\] | 是 | 我已经有的资质 |
...
| `qualType` | [`QualificationType`](#qualificationtype) | 是 | 证件类型。**BUSINESS_LICENSE 是入驻校验的判据** |
| `qualName` | `string` | 是 | 证件名，如「食品经营许可证」。上架校验拿它与类目门槛的文案比对 |
| `qualNumber` | `string,null` | 否 | 证件编号，证上印的那一串 |
| `imageUrl` | `string,null` | 否 | 证件照地址 |
...
| 字段 | 类型 | 必填 | 说明 |
|---|---|:---:|---|
| `regions` | `object`（见下）\[\] | 是 | 命中的行政区划，带从省到父级的路径 |
| `communities` | `object`（见下）\[\] | 是 | 命中的社区 |
...
| `problems` | `object`（见下）\[\] | 是 | 有问题的行：认不出货号、值不合法。**要能逐行看** —— 只给个数商家不知道改哪一行 |
...
| `businessMode` | `SELF_OPERATED` \| `THIRD_PARTY` | 否 | 经营模式（门店级）。自营门店加经营类目不判资质，「调整经营类目」面板因此不标「需资质」 （TDD-门店经营类目）。老后端不发 —— 端上按「不是自营」处理，宁可多提示不少提示。 |
...
| `name` | `string` | 是 | 名称 |
| `address` | `string,null` | 否 | 地址 |
| `adcode` | `string,null` | 否 | 国标 6 位区县码，与 sys_region 同口径 |
| `latE6` | `number,null` | 否 | 有些提示（纯地名、公交线）没坐标，这种不值得选 |
| `lngE6` | `number,null` | 否 | 经度 ×1e6。**全站坐标一律 gcj02** |
| `typecode` | `string,null` | 否 | 高德 POI 类目码 |
...
| `address` | `string` | 是 | 地址 |
| `type` | `string` | 是 | 点的类型 |
| `status` | `string` | 是 | 核实状态；这里只会是 ACTIVE |
...
| `source` | `string,null` | 否 | 来源类型 |
| `status` | `string,null` | 否 | 状态 |
| `tagNos` | `string`\[\] | 否 | **取交集**：选两个标签是「都要满足」。界面上写「同时含以下标签」 |
| `lastOrderBefore` | `number,null` | 否 | 上次下单**早于**这个时刻。用来筛沉睡客 |
...
| 字段 | 类型 | 必填 | 说明 |
|---|---|:---:|---|
| `sourceType` | `string` | 是 | 这一次是怎么来的（扫码 / 分享 / 商家录入 / 活动） |
| `storeNo` | `string,null` | 否 | 发生在哪家店 |
| `linkNo` | `string,null` | 否 | 分享链接号 |
| `inviterUserNo` | `string,null` | 否 | **谁发的链接**。不写出来的话分享激励没法结算，商家也不知道该谢谁 |
| `inviterRole` | `string,null` | 否 | 分享人的身份（顾客 / 店员）—— 结算口径不同 |
| `operatorNo` | `string,null` | 否 | 商家录入时的经手人 |
| `activityNo` | `string,null` | 否 | 来自哪个活动 |
| `isFirst` | `boolean` | 是 | 是不是首次进店。首次那一条决定了这个会员算谁带来的 |
| `occurredAt` | `number` | 是 | 发生时刻 |
...
| 字段 | 类型 | 必填 | 说明 |
|---|---|:---:|---|
| `storeNo` | `string` | 是 | 发生在哪家店 |
| `orderCount` | `number` | 是 | 累计下单数 |
| `totalSpentMinor` | `number` | 是 | 累计消费（分） |
| `lastOrderAt` | `number,null` | 否 | 上次下单时刻。空 = 从没下过单（线索会员） |
| `isFirstStore` | `boolean` | 是 | 他是从这家店进来的 |
...
| `dimQuota` | `number` | 是 | 维度配额上限，按档位取 |
| `valueQuota` | `number` | 是 | 取值配额上限 |
| `values` | [`SpecOption`](#specoption)\[\] | 是 | 这个维度下的取值 |
...
平台活动（原型 s27 · s28 · s29 · s30）。规则与商家活动同一个模型；多出来的只有出资、预算、报名。 出资写成**每单金额**（s28 的约定）：商家看的是「一单我出多少」。
...
| 字段 | 类型 | 必填 | 说明 |
|---|---|:---:|---|
| `minRating` | `number,null` | 否 | 评分下限；空 = 不限 |
| `noViolation` | `boolean` | 是 | 是否要求无违规 |
| `categoryNos` | `string`\[\] | 是 | 只收这些类目的货；空 = 不限 |
| `cityCodes` | `string`\[\] | 是 | 只收这些城市的店；空 = 不限（暂只展示、不校验） |
...
| `mode` | [`AreaMode`](#areamode) | 否 | 覆盖方向：`INCLUDE` 纳入（默认，不传即此）/ `EXCLUDE` 排除。 它回答的是「商家框了小区，算不算覆盖里面每栋楼」—— **默认算**， 但给一个显式的出口：勾了整个小区、单独排除 3 幢。 展开时先并后减，EXCLUDE 优先；而矛盾应当在**输入端**消除 （勾了排除就把对应的 include 去掉），不要求用户记住这条优先级。 |
...
| `title` | `string` | 是 | 标题 |
| `titleI18n` | [`Record_string_string`](#record_string_string) | 否 | 标题的多语言版本 |
...
| 字段 | 类型 | 必填 | 说明 |
|---|---|:---:|---|
| `channel` | [`FulfillmentType`](#fulfillmenttype) | 是 | 履约渠道（快递 / 到店自提 / 邻里自提 / 商家自送 / 上门预约） |
| `enabled` | `boolean` | 是 | 开着没有。**关掉意味着这条路从此不接新单**，已有的单照常履约 |
...
调拨单状态。**与后端 `InvEnums.TransferStatus` 逐字一致。** DRAFT 草稿（还没有行）· SHIPPED 已发出（在途）· RECEIVED 已收到 · VOIDED 已作废。 此前这里是 `status: string`，取值只写在一句注释里 —— 于是 `inv_transfer_order.status` 一直挂在 `known-unregistered-value-domains.txt` 上， 而端上有 7 处 `status === "…"` 的裸字面量比较，拼错一个字母不报错、 那个分支从此不进（在途角标、数量、操作条都挂在 SHIPPED 上）。 VOIDED 是这次补上的：注释里原本只写了三个，而页面已经在比较它了。
