...
> 状态：**已被取代（2026-09-06）· 只当改名决策的历史记录读**。
>
> ⚠️ **这一份不是清单了，它的数字是错的**：标题写 58 实体，而实测 188
> （建表 195 张、188 张挂着实体）。现行清单是生成的：
> [中英文对照 · 实体与字典](./中英文对照-实体与字典.md)，
> 由 `node scripts/gen-glossary.mjs` 从 `@TableName` 与迁移现抽，
> 闸门在 `scripts/check-generated-docs.mjs`。
>
> **本页保留的是「为什么改成这些名字」** —— §一 的三条原则与 §二 的逐个改名决策，
> 那是评审结论，生成器抽不出来也不该抽。要查「现在有哪些实体、叫什么」一律去上面那份。
> 它落后 130 个实体不是写的人不认真，是**手写清单没有变短的机制**：
> 加实体的人不会想起来回来改一份别的文档（与[数据库表清单](./数据库表清单.md)
> 换成生成物是同一条教训）。
