...
## 2026-09-20 收窄：浏览模式只留三样

用户看过真机后定：**尽量简洁，附近不列、搜索先屏蔽，统一通过地图选点**。

于是浏览模式现在是：当前位置（可重新定位、可用）· 我的收货地址 · 地图选点 · 新增收货地址。
「搜索范围 / 切换城市」「搜索框」「附近小区」三段加了 `!browse`，
**实现全都留着** —— 将来要放出来只是去掉这三处 `v-if`，不用重写。
默认模式（建地址那条路）一个字没动。

原型 `c-locate.html` 同步改了：l02 / l05 / l06 按这一版画，l03 搜索标成「将来态」。
