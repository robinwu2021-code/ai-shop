...
| 批 | 内容 | AC |
|---|---|---|
| **P1** | 集单全链路（期、截单、撤单、退款、提货日）；拼团接通下单与到期退款（D1–D6）；新建活动两步化 | AC-1 · 2 · 3 · 5 · 6 · 7 · 8 · 9 · 10 · 11 · 15 |
...
1. **拼团接通下单放进 P1**（D1）：不接的话 P1 做完拼团仍然收不到钱，AC-6 的「到期退款」也无从谈起。这会让 P1 的工作量明显变大。
2. **「退差价」玩法删除**：C 端参团成功提示里那句「先买的邻居也退了差价」没有任何实现，本设计按下单即收团价处理。
3. **平台活动的 `entity_no` 为空**：P3 要放开这一列的 NOT NULL，需要先确认数据域对空锚点的处理（今天是 fail-closed，平台活动会对所有商家不可见）。
