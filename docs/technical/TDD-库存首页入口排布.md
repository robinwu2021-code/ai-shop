...
- **AC3** 总览卡里那排等大文字链接不再存在；「看各店的货」挪到四个数下面，
  仍然只对开了不止一家店的商家出现。**总览卡从此只有数**。
- **AC7** 整页**只有一个地方能点动作**：「收货 N」「继续盘点」也收进底下那块面板，
  加在按钮上面 —— 有事时多出来的行往上长，底下那排按钮贴着屏幕底边，位置一动不动。
...
| AC7 | 动作只有一个区域 | `urgent`（`urgentStockItems()` 不变）搬进 `.bar` · 占位高 `barPad` 跟着条数长 |
...
| 「有人在等」 | **并进底下那块面板**：收货 1 › / 继续盘点 ›，在按钮上面一格。整页只剩一处能点动作 |
| 占位高够不够 | 两种状态各量一次：有两条在等时面板 132px、富余 48px；一条都没有时面板 48px，都**没压住最后一张卡**。`barPad` 是 `140 + 条数 × 88`（rpx），两个数都是量出来的不是估的 |
...
- **「有人在等」原本单独一张卡**，商家看过之后并进底下那块面板：
  「按钮放在不同的区域，很零散」。往上搬不行（进货每天按几次，不能离开拇指），
  所以往下并。顺带修好一件事：它原来在滚动流里，**列表一滚就没了** ——
  真有货在等的时候反而看不见；现在常驻在屏幕上。
- **菜单原本三组六条、每条一句说明**，商家看过之后只留名字。
  `stockEntries()` 的返回值跟着从 `groups`（分组）改成 `more`（一条平列表）——
  留着分组结构而界面上不画，等于留一段没人读的代码。

### 中途红过两道闸门，都不是这次改红的（现已全绿）
...
**① 四个数字点了没反应。** `sh-stat` 只 emit `change`，而这一页写的是 `@pick`
（那个名字留给 picker 一族）。于是「在售 SKU / 缺货 / 滞销 / 在途」四个入口
**从来没有工作过**，而文件头那句「三个数字即入口」一直写着它是能点的。
不报错、`vue-tsc` 也看不见 —— 未声明的 `@pick` 只是个落到根节点上的普通属性。
H5 mock 上两向验过（撤回修复 → 点了仍是 4 条、无高亮；修好 → 缺货 2 条、滞销 1 条、在途跳单据）。
**每次点之前先 reload** —— hash 跳转复用页面实例，不重载量到的是上一次的筛选，第一次就栽在这儿。

**② 有盘点单开着时，「盘点」这个入口让位给「继续盘点」。**
后端 `open()` 一句「已经有一张开着」都没查，而 `openCountNo` 只给最近的那一张
（`ORDER BY id DESC LIMIT 1`）。两张单锁的是两个时刻的账面数：先开那张过账时，
会把这中间卖掉的量当成盘亏再扣一遍，**账朝一个方向错且不报错**。
判据放在两页共用的 `stock-urgent.ts`（`canStartNewCount()`），库存页与工作台各接一处；
两处都做了消融（撤掉判断 → 「继续盘点」与「盘点」当场并排出现）。
**后端那道闸没做** —— 要新错误码，动契约，另算。
...
| 2026-09-17 | 第三次返工：「收货／继续盘点」并进底下那块面板，页面上只剩一处能点动作 |
| 2026-09-17 | 补两处：①四个数字点了没反应（`sh-stat` 发 `change`、这一页听 `pick`）②有盘点单开着时「盘点」让位给「继续盘点」 |
| 2026-09-17 | 已实现。跑过的闸门：`vue-tsc`（b-app，exit 0）· `vitest`（b-app 40 条全绿）· `gen-ui-catalog.py --check` · `gen-inv-ui-inventory.py --check` · `check-i18n-orphan.mjs --check`（0 回归）· `check-shared-guards.mjs --check`（**全绿**：本次引入的三条已修，HEAD 里别人那两条也已由 `66eacf19` 修掉）· `check-rtl-physical.mjs --check`（0 处） |
