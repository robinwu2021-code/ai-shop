...
- [x] T8 押后项登记 —— **只剩一项**（见 §7.4）：迟到回调自动退款已登记进
      [支付通道对接-接口清单](支付通道对接-接口清单.md) §6.1；
      C 端倒计时那项**本就不存在**
...
### 7.4 「C 端倒计时读的是常量」——这条也不成立

方案 §3.2 与 §3.5 把「C 端倒计时改读 `payDeadlineAt`」列为跨端押后项，
理由是端上目前读常量、配置一改就会「用户看着还剩 3 分钟，订单已经关了」。

实际 `c-app/src/pages/pay/index.vue` **一直读的就是 `order.payDeadlineAt`**。
唯一出现 `TRADE_RULES.payTimeoutMinutes` 的地方是 `c-app/src/api/mock.ts`，
它在那里**替服务端算这个字段** —— 那正是 mock 该做的事，不是缺陷。

连同 §7.1（关单能力早就存在），这是同一类误判：**方案阶段按关键词搜了一遍代码，
搜到的位置对，但没看它在那个位置扮演什么角色**。
`closeExpiredOrders` 搜不到调用方被读成「功能没有」，
`payTimeoutMinutes` 搜到在 c-app 里被读成「端上在用它算倒计时」。

一个便宜的对策：这类判断落笔前，把命中的那一行**连同上下文五行**贴进方案，
而不是只写结论。贴出来的话，`mock.ts` 那一行旁边就是 `payDeadlineAt:`，
自己就说明了它是服务端字段的替身。
