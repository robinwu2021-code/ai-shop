...
### 3.4 邮件的消费方：本批只建通道，不造场景

邮件**今天没有任何业务消费方**，需求文档里也搜不到「邮件」二字。
本批只交付 `MailPort` ＋ SMTP 实现 ＋ 一条自检端点（`/actuator` 或 ops 侧手动触发），
**不自行发明业务场景**（如"订单通知邮件"）——那要走需求。

理由：造一个没人要的通知场景，等于让 `system@neargo.ai` 开始给用户发信，
而没有人定过发什么、多久发一次、怎么退订。
...
- [ ] T7 全量回归；文档记录「打开开关的前置条件」
- [ ] （可选同批）T8 闸④验码失败锁定 —— 缺陷 B 本体
