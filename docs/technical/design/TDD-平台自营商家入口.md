...
> 状态：**一期已落地 · 二期（自营免证件 + 运营建自营店）实现中 · 三期（代商家建）待评审**
> 创建 2026-09-16
...
**① 替真人开账号。** `ensureUserByPhone` 会给这个手机号建一个账号，而**本人当时并不知道**。
自营入口这么做没问题（那是平台自己的号），代商家建就不一样了。三个选项：

- (a) 照建，商户首次登录时看到「你的资料由运营 X 于某日代为提交」并需确认；
- (b) 只建申请单、不建账号，等商户自己首次登录时再绑；
- (c) 代填之后给该手机号发一条短信告知。

我倾向 **(a)+(c)**：账号照建（否则审核通过时没有 owner 可挂），但**商户第一次进来
必须看到这件事**，而不是发现自己名下凭空有一家店。

**② 协议同意。** 今天的入驻流程里有「我已阅读并同意《商家服务协议》」那一勾。
**运营不能替商户勾它。** 建议：代填的申请单上记 `agreed_by = null`，
商户首次登录时补勾，未补勾前**可以经营但不可提现**（或你定别的界线）。
这一条不定下来，代建出来的每一家店在法律上都悬着。

### L3 · 要动的地方

| # | 位置 | 做什么 |
|---|---|---|
| 1 | `OpsMerchantApplyController`（新） | `POST /ops/merchant/apply/on-behalf` |
| 2 | `Perms` | 新码 `merchant:apply:onbehalf`，给 BD 与超管（**与 `selfop:create` 分开** —— 那个只给超管） |
| 3 | `SelfOperatedService#addStore` | 第三方那一支改走 `StoreAdminService.create`（额度生效） |
| 4 | `mch_entity_apply` | 加 `submitted_by`（代填人）与 `agreed_at`；两者都影响上面那两条决定 |
| 5 | ops-web | 代填表单（复用入驻审核页的字段）+ 门店新增 |
| 6 | 跨域登记 + 生成物 | 见第一期 L3 |
