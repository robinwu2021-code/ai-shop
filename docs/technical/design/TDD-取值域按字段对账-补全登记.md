...
状态：**草稿 · 待修正** · 创建 2026-09-06
> ⚠️ **§2.2 的候选来源方向反了**，动手前先看
> [取值域治理-整改清单](取值域治理-整改清单.md) P4。
> 本篇的候选取自 DDL 列注释 + Java 常量组 + 端上取值域 —— 全是代码，于是
> **需求里有、代码从没实现的状态永远进不了候选**，裸字面量的取值域也进不来。
> 正确方向是：需求端整理领域对象 → 每个对象的状态/类别 → 再和代码对齐。
> 其余各节（§1 现状、§3 选型的四个反例、§4 风险、§5 AC）仍然成立。
