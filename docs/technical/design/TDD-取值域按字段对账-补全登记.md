...
| AC | 测试方法 | 跑过 | 消融验证 |
|---|---|---|---|
| AC1 | `enum-fields.test.ts › 未判定的取值域列只准变少`（同一条断言兼顾点名） | ✅ 2026-09-06 | 从 `FIELDS` 删掉 `prd_goods.type` → 它进未判定面且不在基线里 → 红并点名 |
| AC2 | `enum-fields.test.ts › 未判定的取值域列只准变少 —— 新出现的要点名` | ✅ 2026-09-06 | 从基线删一行 → 红并点名该列与它的取值 |
...
**AC2 的判据是「按 key」不是「按数」**（2026-09-06 实现时收紧）：只盯数量的话，
判定掉一条、又新加一列带取值注释的表，数字不变而新的那条溜进来了。
棘轮文件里存的因此是 `表.列` 的清单，不是一个计数 ——
这同时避免了「基线与量具量的不是同一个数」那类收紧后恒红。
