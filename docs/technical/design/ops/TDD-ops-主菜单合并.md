...
> **2026-09-09 追记：已处置，脚本删除。** 单独跑了一次才看清它坏得比这里写的更深：
> 除了后端码陈了（导致 10 个非超管角色**每一格**都算成「无权限」），
> 它的 `ROLE` 表里还有 `MERCHANT_BD` / `PRODUCT_OPS` / `CS` 三个
> **在 `Perms.java` 里根本不存在**的角色码（真名 `BD` / `GOODS_OPS` / `SUPPORT`）——
> 11 列里 3 列的键名是虚构的。而它照样跑完、照样吐出格式正确、行数对得上的 127 行表。
>
> 处置不是修它，是**并进 `gen-perm-seed.mjs --matrix`**：矩阵与权限种子从此
> 共用同一个解析器（角色从 `Perms.ROLE_PERMS` 现读，不抄）。
> 合完做了一次交叉核对 —— 矩阵里每个角色的 ● 数与种子 SQL 里
> `sys_role_point` 的菜单类授权数，11 个角色里 10 个逐字相等；
> 唯一的差是超管 122 vs 124，正好是那 2 个 `UNIMPLEMENTED` 项
> （入库但对所有人隐藏，§3 写明的语义差）。
> 产出的文档 `运营端-动态菜单×角色矩阵.md` 已登记进 `check-generated-docs`，
> 也就是本条偏差说的「挂不上」现在挂上了。
