...
> 用表格而非 ER 图：mermaid 写错一个字符整张图静默不渲染（`doc-standard.test.ts` 有闸）；
> 数据字典类内容「左表－关系－右表」逐行可查。全量关联见 [v2/15 §二](../../v2/15-商品订单数据表总册.md)。

| 左表 | 关系 | 右表 | 键 / 说明 |
|---|---|---|---|
| `cat_product_type` | 1─N | `prd_goods` | `product_type_no` |
| `prd_category` | 1─N | `prd_goods` | `category_no`（恰好一个） |
| `prd_goods` | 1─N | `prd_sku` | 唯一 `(goods_no, market, option_values)` |
| `prd_goods` | 1─0..1 | `prd_trait_service` | 只有挂了该 Trait 的商品有行 |
| `prd_sku` | 1─0..1 | `prd_trait_metered` | 挂 SKU（整鱼/切段标称重不同） |
| `prd_sku` | 1─N | `prd_metered_rate` | 费率段 |
| `prd_goods` | 1─0..1 | `prd_trait_preorder` · `prd_trait_bundle` | 各自 1:0..1 |
| `prd_trait_bundle` | 1─N | `prd_bundle_component` | 子项必须同主体 |
| `prd_modifier_group` | 1─N | `prd_modifier` | |
| `prd_goods` | N─N | `prd_modifier_group` | 经 `prd_goods_modifier_group`，组可复用 |
| `prd_goods` | 1─N | `prd_store_listing` | 门店维度 |
| `prd_sku` | 1─N | `sell_price_entry` | 渠道×客群×数量档×有效期 |
| `prd_store_listing` | N─N | `sell_availability_rule` | 经 `sell_listing_availability`（挂 listing 不挂 goods） |
| `sell_collection` | 1─N | `sell_collection_item` | |
| `mch_station` | 1─N | `prd_store_listing` | `station_no`（商户域·外） |
