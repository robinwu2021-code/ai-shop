...
### 切换时点（2026-09-02 定）：与 pay-svc 的 native 部署同批

> 拍板时只说了「将来一定会切」，没说什么时候。现在有时点了：
> **等 pay-svc 稳定之后，切 MySQL 与它的 native 部署一起做。**

不是凑批次 —— 两件事共用同一个前置：**C2c 把持久层从 MyBatis 换成
Spring Data JDBC**。换完之后，native 的最大成本项（MyBatis-Plus 没有原生镜像
元数据）消失，而本文档 §1 里那些方言风险也从 113 处收敛到 4 处手写 CAS ——
其余的 CRUD 由 Data JDBC 的 dialect 生成，MariaDB 与 MySQL 的 dialect 差异极小。

分开做要把持久层动两次，而换持久层丢的是**隐式行为**：不报错、不影响编译，
要等某条真实路径被真实数据走到才现形。动两次就承担两次这种风险。

判据与详细排布见 [TDD-GraalVM 与资源占用优化 §L4-2b](TDD-GraalVM与资源占用优化.md)。

**在那之前本文档 §2 的规矩照旧生效** —— 新写的 SQL 两边都要能跑。
时点推后不等于可以先欠着：欠账要在切换那天一次性还，而那天是最忙的一天。
