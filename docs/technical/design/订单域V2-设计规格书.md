...
CREATE TABLE ord_order (
  ...
  supplement_of VARCHAR(32) NULL COMMENT '本单是哪张订单的补款单；NULL=普通订单（已付订单正向差额不改原单，另开补款单）',
...
CREATE TABLE ord_item (
  ...
  qty INT NOT NULL,
  shipped_qty INT NOT NULL DEFAULT 0 COMMENT '已发数量（分批交货，电子元器件回填）：=qty 时行闭合，全行闭合子单才 COMPLETED',
...
CREATE TABLE ord_adjustment (
  ...
  kind          VARCHAR(16) NOT NULL COMMENT 'DISCOUNT 让利/ADJUST 补差/REPRICE 重定价/CORRECTION 更正——四类下游影响不同，混作一谈是最大的坑',
  reason_code   VARCHAR(24) NOT NULL,
  reason_text   VARCHAR(255) NULL COMMENT '免单与议价**必填**（无原因的减免是内控漏洞）',
  amount_minor  BIGINT      NOT NULL COMMENT '有符号：负=少收，正=多收',
  new_unit_price BIGINT     NULL COMMENT '仅 REPRICE：新单价（原行 price 不动）',
  qty_from INT NULL, qty_to INT NULL COMMENT '仅 REPRICE：生效数量区间（分批改价）',
  supplement_order_no VARCHAR(32) NULL COMMENT '正向差额生成的补款单',
  approved_by   VARCHAR(64) NULL COMMENT '超门店阈值需审批',
...
) COMMENT='订单调整：只追加不覆盖。有效金额=原始+Σ调整，现算不落库。详见 TDD-订单改价-通盘设计';
...
| 从 | 可迁移到 | 触发与约束 |
|---|---|---|
| `WAIT_PAY` | `PAID` · `CANCELLED` · `CLOSED` | 支付成功 / 主动取消 / **超时关单** |
| `WAIT_OFFLINE_PAY` | `PAID` · `CANCELLED` | 确认收款或 settle / 超时取消（**按门店配置，台账未结不取消**） |
| `PAID` `CANCELLED` `CLOSED` | —— | 终态；回调只推进不回退（先到的终态胜出） |

**没有 `WAIT_OFFLINE_PAY → WAIT_PAY`**：改主意要重新下单 —— 允许来回切，
「这单收没收到钱」就有了两个真源（商家确认 与 支付回调），而它们可能同时到达。
**子单**（`ord_sub_order.status`）

| 从 | 可迁移到 | 说明 |
|---|---|---|
| `WAIT_PAY` | `WAIT_FULFILL` · `FULFILLING` · `CANCELLED` | 实物落 WAIT_FULFILL；**SERVICE_LIKE（核销/预约/堂食）直落 FULFILLING** —— 商家没有备货发货这一步 |
| `WAIT_FULFILL` | `FULFILLING` · `COMPLETED` · `REFUNDED` | |
| `FULFILLING` | `COMPLETED` · `REFUNDED` | |
| `COMPLETED` | `REFUNDED` | **售后可以发生在完成之后** |
| `CANCELLED` `REFUNDED` | —— | 终态 |
**售后**（`ord_after_sale.status`）

| 从 | 可迁移到 | 说明 |
|---|---|---|
| `APPLIED` | `REFUNDING` · `REFUNDED` · `REJECTED` · `CLOSED` | 受理按商品 `after_sale_policy` 分流 |
| `REFUNDING` | `REFUNDED` · `CLOSED` | |
| `REJECTED` | `ARBITRATING` · `CLOSED` | 驳回后用户可申诉 |
| `ARBITRATING` | `REFUNDING` · `REFUNDED` · `CLOSED` | 平台裁决 |
| `REFUNDED` `CLOSED` | —— | 终态 |

**收款**（`ord_payment.status`）：`PENDING → SUCCESS | FAILED`；`SUCCESS → REVERSED`（**仅一次**，O7）。
