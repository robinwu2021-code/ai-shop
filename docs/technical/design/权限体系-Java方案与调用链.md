...
登录时算权限快照的那一步已改成 `RolePermResolver.of(roles)`：
读 `sys_role_point → sys_function_point.perm_code`，整表缓存一次。
`Perms.of` 仍在，作为**回落**：库里查不到这个角色、或查到但为空时用它 ——
「没配」与「配了零权限」必须分开，后者会让一个本该有权限的岗位静默失权。

**换源之后有一条新的断言**：删掉库里 BD 对 `quote:govern` 的授权，
他登录后拿到的 `perms` 真的少了那个码 —— 不变就说明还在读硬编码，
这次换源没有真正发生。
