...
**关于批次的唯一键** —— 这里连着写错过两次，两次都记下来：

**第一次**写「今天的键是 (主体 × 通道 × 应结日)，目标是加币种」。
查实际 DDL 后改成「批次表上唯一的键是批次号本身，那个组合只是查询条件，
今天没有任何约束防重复开批」。

**第二次那句也是错的。**实施时才发现 `uk_stl_batch_period
(entity_no, pay_channel, period_from, tenant_no, deleted)`
<b>V280 建表时就有，线上也有</b> —— 它防的正是「截批任务重跑一次
就把同一区间的单开出两个批」。

起因是用 `grep -A30` 看表定义，而这张表有 27 列，**约束在 30 行之外**。
<b>窗口太小导致的假结论</b>，而它差点变成一条与现有约束同名的
`ADD CONSTRAINT` —— 那会让生产迁移直接失败。

**实际做法**（V287）：把币种加进这道已有的键，保留 `deleted`
（软删除下让已删批次不占键位的既有语义）。
写法用 `DROP INDEX … ON` + `CREATE UNIQUE INDEX`，
不用 `ALTER … ADD CONSTRAINT` —— <b>测试库的 schema 生成器只认前者</b>，
用后者那道键会在 H2 schema 里静默消失。
