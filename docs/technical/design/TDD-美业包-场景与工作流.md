...
> **2026-08-28 修订（第二批）**：`bty_appointment_slot`（占位明细/回退单位）与 `bty_schedule_log`（调班留痕）
> **作废**，分别上收基座 `mch_booking_hold(+unit)` 与 `mch_schedule_change_log` —— 原子占位这种并发要害
> 只在基座实现一次。见 [TDD-预约资源域-多行业完整方案](./TDD-预约资源域-多行业完整方案.md)。正文保留原样。

> **2026-08-28 修订**：本册的收款流水表 `bty_payment` **作废**，上收基座 `ord_payment(ref_type=WORK_ORDER)`。
> 见 [TDD-订单域-多行业完整方案](./TDD-订单域-多行业完整方案.md) §三。正文保留原样 —— 历史不改写。
