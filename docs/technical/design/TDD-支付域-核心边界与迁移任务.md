...
M1–M9 理的是「谁依赖谁」。另有三条是**领域模型本身缺的**，
与拆分无关 —— 不拆也该有，拆了更要有：

| # | 要求 | 落地前的实际状况 | 做法 |
|---|---|---|---|
| **P1** | 一笔订单对应一笔或多笔支付流水 | 商户单号恒等于订单号且永不变，**一笔订单只能向通道下一次单** | 商户单号 = 订单号 + 尝试序号（首次即订单号，重试 `-2`、`-3`） |
| **P2** | 支付流水对应渠道发送或回调报文 | **一行都没存**。被拒的四条路径各自只有一句 `log.warn` | 新表 `stl_channel_message`（V286） |
| **P3** | 交易记录的结算与对账按渠道进行 | 结算侧已按渠道分批；**对账侧只有总数** | 对账结果带按渠道分解 |

三条各自的关键取舍，写在代码注释里而不是这儿（改代码的人不会先来读文档）：

- **P1 为什么不做成完全独立的流水号**：先做了那一版，
  **20 多个测试类同时变红** —— 它们测的是业务链路，不是支付细节。
  加后缀既拿到了「可以换号」，又让最常见的那条路径一个字都不用改。
  这是先量了影响面才改的。
- **P2 为什么没验签的报文只存指纹**：回调端点公网可写，
  原样落库等于给了个免费的写入口。指纹回答的是排查时真正要问的那句话 ——
  通道在重推同一份，还是有人在扫端点。
- **P2 为什么报文要独立事务**：处理失败的那一次恰恰是最需要报文的那一次。
  跟着调用方事务走，表里就只剩顺利的那些。
  （第一版的守卫测的其实是「controller 不在事务里」，消融不红，已换掉。）
- **P3 为什么值得做**：「三十笔在回调路上」与「微信整个查不通」
  在总数里长得一模一样，而处置完全相反。

顺带纠正一处旧账：`stl_payment.raw_notify` 从 V1 建库至今**一行都没写过**，
而它的列注释写着「唯一的资金凭据 —— 出纠纷要原文」。
一个宣称自己能举证、而实际永远为空的列，比没有这一列危险。
职责交给 P2 的新表，注释已改成说真话。
