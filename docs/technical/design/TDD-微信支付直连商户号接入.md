...
## 13. 联调现状与交接（2026-09-05 14:30，供下一个会话接手）

### 13.1 已证实的事实（都有对照量）

| 层 | 结论 | 证据 |
|---|---|---|
| 商户号 ↔ 小程序绑定 | **通** | 用真 openid 以外的串下单，微信回 `PARAM_ERROR 无效的openid`；绑错 appid 回的是另一种错 |
| 统一下单 | **通** | 线上日志反复出现 `[wechat-direct] 下单成功` + `端上六件套 … paySign自验=通过`（最近：`SO202609051408370025441`，10 分） |
| 微信侧订单 | 存在，`NOTPAY` | 查单接口按 out_trade_no 回读 |
| 回调入口 | 在 | `POST /pay/callback/WECHAT` 与 `/refund` 都回 `{"code":"FAIL"}`（验签 bean 已装配） |
| 端上 | **`requestPayment:fail banned`** | 每次下单都在这一步止住；没有任何回调进来 |

`banned` 是微信客户端在打开收银台**之前**就拒绝——不看商户号、不看签名，
看的是**小程序自己有没有支付能力**。换 1750232333 结果一样。

### 13.2 根因判定：小程序曾加过「商家自营 > B2b(商品批发/门店管理)」类目

社区多例一致：加过 B2b 类目却没接 B2b 商户号，微信把小程序限定为「仅 B2b 支付」，
普通 `requestPayment` 恒 `banned`；**删类目不自动解除，要类目客服人工撤标记**。

2026-09-05 逐页只读核过（小程序后台 + 商户平台），除此之外无任何可改项：

| 检查点 | 状态 |
|---|---|
| 服务类目 | B2b 已于 14:00 删除；剩家政 / 生鲜 / 3C / 个护家清 / 玩具 |
| 微信支付关联 | 1117261658、1750232333 均「已授权」，待关联表为空 |
| 「接入微信支付」栏 | 「已申请」= 当初误申请 1750232333 的记录，不影响 |
| 交易结算管理授权 | 已完成（09-05 上午） |
| 订单管理 / 发货信息管理 | 已接入（实物电商类，快递 T+10 / 其他 T+2） |
| 通知中心 16 条 | 无支付受限通知；认证 09-03 刚通过，名称 09-02 改为「虹选」 |
| 商户平台产品 | JSAPI 支付 / 刷脸 / 自动提现 已开通 |
| API 安全 | 证书已申请（2031-09-03 到期）、公钥已下载（09-04）、APIv2 密钥未设置（不需要） |
| 类目客服 | 14:07 在「小程序类目审核咨询」发了申诉，尚无人工回复 |

### 13.3 接手时先做什么

1. **看客服有没有回**（mp.weixin.qq.com → 类目页「联系客服」窗口，或通知中心）。
   回了/或隔天：手机完全退出微信 → 体验版 → 商品 G9901「【支付联调】0.1 元测试商品」下单 → 立即支付。
2. 成了就按 §7.4 验：日志 `SEND OK` → `CALLBACK ACCEPTED`、`stl_payment` SUCCESS + `payer_openid`、
   商户平台交易中心能查到 trade_no。然后**删测试商品 G9901/SK9901**。
3. 还是 `banned`：在开放社区微信支付板块开贴（附 appid、mchid、out_trade_no、删类目时间），
   不要再改代码——两侧配置已确认没有可改项。

### 13.4 未完成的欠账（与 `banned` 无关，支付一通就该做）

- **发货信息录入接线**：通道层（`WxShippingGateway`）、台账（`trd_shipping_upload`，V323）、
  服务（`WxShippingUploadService`）都在，**尚未挂到** 3 处 `setStatus(FULFILLING)` 与服务类 ORDER_PAID；
  重试 job 未写；接线后要按生产 profile 冒烟。mchid 用 1117261658。
- 对账轴误报：未支付被算进「留待下轮」，每轮都喊「WECHAT 全部判不了」——要把「未付」与「查不通」分开。
- `adoptOrReject`（手机号已绑另一账号时的收编逻辑）写了但接成 `throw CONFLICT`，等产品定边界。
- `POST /mp/wx/callback` 验签是同伴的功能，只有守卫钉着「不许先做副作用」。

### 13.5 小程序发版（供本文档自足）

```bash
cd c-app && npm run release:mp -- <版本号> "<备注>"   # 构建 → 校验 → 传服务器 → 上传开发版
```
