...
**待决（不由我定，且这次之后我不会再自作主张动内存）**：

| 方案 | 怎么改 | 代价 |
|---|---|---|
| A. 抬限额去就 buffer pool | `MemoryHigh=1536M` `MemoryMax=2G` | 承诺内存从 52% 升到约 70%，机器有余量 |
| B. 降 buffer pool 去就限额 | `innodb_buffer_pool_size=320M` | 三库共 296M，仍够整库常驻；改动最小 |
| C. 维持现状 | — | buffer pool 名存实亡，每次部署再挤出去一批 |

**B 最省事且符合实际数据量**，但两者都要改完重启 MySQL —— 那是一次计划内的中断，
不该顺手做。**`swapoff` 在有 cgroup 限额的机器上不要再用。**
...
**未修**：这是 `shop-job` 的应用代码问题（大概率是非守护线程或 Spring 关停钩子卡住），
不在这份运维方案的范围里，另开。
