...
> 状态：**M0–M5 已实施（2026-09-14）· 清理与巡检未做 · 兼容软链未删** · 创建 2026-09-14
...
| `/data/backup/ai-shop/legacy/` | **只报不删**，人看过再删 | 1.4G：18 个 8-24/8-25 的 `shop-app.jar.bak-*` + 两个悬空软链 |
...
| 没规划旧 jar 备份 | 新增 `/data/backup/ai-shop/legacy/`（1.4G） | `/opt/ai-shop` 里有 18 个版本化部署之前手工留的 `shop-app.jar.bak-*`；搬出服务目录、集中放，只报不删 |
...
1. §4 清理脚本（先 dry-run 一周）与 §6 本机巡检 —— 巡检的通知出口待定（L4 第 1 条）
2. 腾讯云云监控的磁盘告警策略（控制台操作）
3. 一个完整部署周期后删兼容软链
