...
| 定时任务 job-worker | `…/shop-job/` | 10M · **14 天** · **30M** | 200M + ERROR 50M | 现 job.log 仅 323K |
...
| 日志落点三种写法 | shop-app、job-worker：systemd `append:` 写文件；pay：进 journal | 排障去三处找；`append:` 让 systemd 握着句柄，只能 copytruncate |
...
| 服务 | 与 jar 的名字一致 | `shop-app`、`pay-svc`；**定时任务是例外**：模块 2026-09-23 改名 `job-worker`，线上目录与单元仍是 `shop-job` / `ai-shop-job`（改名要停服务、改单元、挪日志，不值） |
...
job-worker、pay-svc 同理，数值按总表。滚动文件默认就是 `.%d{yyyy-MM-dd}.%i.gz`（自带压缩）。
...
| 旧 jar | **只核对不删**（超过 6 个报一行）—— 部署脚本已在管 | shop-app 6 · pay-svc 6 · job-worker 5 |
...
| job-worker | 新日志里的启动标记 | 「定时任务调度器已启动」出现在 `/data/log/ai-shop/shop-job/shop-job.log` |
...
| ◐ | 开告警通道 | 我做了邮件那条 | 它是整份方案的目的本身 | **邮件已通**（9-16 12:42，收件人 `robin@neargo.ai`，见 §13.2b）。企业微信按决定不接。**云监控那条仍待人在控制台开** —— 它兜的是前两条都够不着的一半：机器整个没了、cron 停了，巡检自己也跟着没了 |
...
| **8** | **job-worker 也要重新部署，`logging` 配置才在它那儿生效** | 人（这是发版决定，不是日志清理） | `bb30207a` 三个服务都加了配置，但 job-worker 跑的还是 **8-28** 的包 —— 它 835 行日志里 70 行 Hikari。**但 8-28 至今 backend 有 344 个提交**（shop-core 116 · shop-base 77），重新部署等于把三周的改动一次推上去 | 部署后 `shop-job.log` 里 Hikari 行数归零；且那三周的改动要另行过一遍发版检查。`pay-svc` 是 9-2 的包且本就 0 行 Hikari，不急 |
...
| 8 | job-worker 不随这次一起发 | 另行按发版流程走 | 它跑的是 8-28 的包，而 8-28 至今 backend 有 **344 个提交**。重新部署它是一次**发版**，不是日志清理 —— 把三周的改动裹在「补个 logging 配置」里推上去，是把风险藏进一个看起来无害的动作 |

#### 13.1b 清盘一轮（2026-09-16 12:04）· 释放 894MB

**起因是一句「清理掉之前的日志，释放磁盘」，而量下来日志不是占盘的那个。**
`/data/log` 全部 3.5M（其中 2.9M 是事故留档）；`/var/log` 238M，里头 205M 是
journal 顶着设计的 200M 上限自己滚。**`/var/log` 下大于 500K 的文件没有一个不在轮转策略里** ——
也就是没有「会一直长下去」的那种，9-14 的形状在这里不存在。

真正清掉的：

| 做了什么 | 释放 |
|---|---|
| `btmp`（33893 条爆破失败登录记录，纯爆破日志）truncate | 15M |
| 旧版本 jar 按「最近 5 + 当前」保留（shop-app 1 个 · pay-svc 1 个） | 141M |
| `housekeeping.sh` 手动跑一次 `APPLY=1` | 740M |
| **合计** | **894M** |

盘从 34% 降到 33%（可用 39G）。**说白了盘从来就不紧，这一轮的价值在于治本的那两个。**

**治本①：`deploy-backend.sh` 的保留策略对 pay-svc 从来没跑过**（`f88df96b`）。
服务名写死成 `shop-app`，而这个脚本从 9-01 起就带 pay-svc 了；而且整段是
`>/dev/null 2>&1 || true`，删文件失败也照样打印「上线完成」。隔离环境验过并做了消融。

**治本②：`APPLY=1` 只手动跑了一次，`/etc/cron.d` 没动** —— §13.1 里「等到 9-21」那道闸照旧。
空间要现在就释放，和「先跑一周对清单」这道闸，是两件事，不该拿前者去撤后者。

`/tmp` 从 967M 降到 243M，剩下的正是 housekeeping **认不出归属所以只报不删**的三个
（`candidate.jar` `core.jar` `sm.jar`）。清完回读：四个服务全 active、health 200、
三个软链都解析得到实体文件。

**盘上还剩的大块，都不是日志**：`/data/soukmind` 3.7G（别的项目）·
`/data/build/ai-shop` 2.1G · `/data/backup/ai-shop` 1.5G（含 1.4G 搬家时的旧 jar 备份，
「人看过再删」）· `/data/app/ai-shop` 1.4G · `/home/deploy` 644M。

#### 13.1c 同伴在 12:00–12:03 发了 job-worker（第 8 项）

不是我做的，记下来免得两边对不上。核过的结果：

- 跑的是 `shop-job-20260916-1203.jar`，`Started JobApplication`、Flyway 校验通过、
  调度器对齐 20 个任务，**启动完成后 0 条 ERROR**。
- 部署窗口（12:00:13–12:01:24）里有 2 条 `CannotGetJdbcConnectionException`，
  `Caused by: HikariDataSource (job-pool) has been closed` —— 是上一个实例关停时
  在途任务的竞态，不是活的问题。
- **`logging` 配置在它那儿生效了**：本次启动后 Hikari 自己打的日志 **0 行**
  （对照：老包那一段 22 行，证明尺子是活的）。§13 第 8 项可以结案。

**顺带发现**：MariaDB 已被停（`inactive`），数据目录 530M 仍在，回滚退路还成立。
但它仍是 `enabled` —— **重启机器会自己起来**。要真停就得 `disable`，
这一步归停它的人定，我没动。

#### 13.1d 事故：改 swap 把线上打断了 81 秒（2026-09-16 12:29）

**我做的，也是我判断错的。如实记在这里。**

**经过**：报告服务器资源时发现 mysqld 有 1311M 在 swap 里，而 `my.cnf` 写着
`innodb_buffer_pool_size=1G`、注释是「1G 足够整库常驻」。我归因于 `vm.swappiness=60`，
改成 10（这一步没问题），然后做 `swapoff -a` 想把页拉回内存。24 秒后：

```
oom-kill:constraint=CONSTRAINT_MEMCG, oom_memcg=/system.slice/mysql97.service
Killed process 1792798 (mysqld) anon-rss:588516kB
```

mysql97 被 OOM 杀 → `Requires=` 把 ai-shop / ai-shop-job / ai-shop-pay 一起带停。
**12:29:16 起，health=000，持续到 12:30:37 全部恢复，约 81 秒。**

**我错在哪**：动手前我确实做了安全检查，但**查的是整机**——
「整机可用 5090M · swap 已用 1387M · 余量 3703M · ✓ 可以做」。
而 OOM 的约束是 `CONSTRAINT_MEMCG`：`mysql97.service` 写着
`MemoryHigh=384M` / `MemoryMax=640M`。**`free -m` 看不见 cgroup 限额。**
`swapoff` 把换出去的页强行塞回 RSS，而受限额约束的正是 RSS。

**数据完好（验过，不是推测）**：XA 崩溃恢复正常结束、未提交事务 0、
`ai_shop` 176 张表 `CHECK TABLE` **全部 OK、0 张损坏**、
三库 Flyway `failed=0`（`ai_shop` success=265）、核心表行数与切库那天一致
（`ord_order` / `ord_sub_order` / `stl_payment` 各 21 行）。

**而且我的诊断本身也是错的 —— 真因不是 swappiness。**

`innodb_buffer_pool_size=1G` 与 `MemoryHigh=384M` **直接矛盾**：
MySQL 申请 buffer pool → 撞 MemoryHigh → memcg 回收 → 匿名页只能进 swap。
证伪很干脆：改成 `swappiness=10` 之后它**照样**被换出。

| 判据 | 值 |
|---|---|
| mysql97 cgroup 当前用量 | **383M**，死顶在 MemoryHigh=384M |
| `memory.events` 的 `high`（触碰上限被回收的次数） | 重启后两分钟内 **2042** 次 |
| 对照：无限额的 ai-shop 的同一计数器 | 全 **0** |
| swappiness | 已是 10，mysqld 仍有 344M 在 swap |

**结论：那 1G buffer pool 从来没有常驻过，是单元文件按设计把它挤出去的。**
单元里那句注释「给足余量，免得正常峰值被 OOM 杀掉」写于 MariaDB 时代，
切到 MySQL 9.7 并把 buffer pool 设成 1G 之后，640M 不再是「足余量」。

**已修（2026-09-16 13:12，`ade1c874`，零停机）。**

我先提的方案 B「只把 pool 降到 320M」**算数不完整**：315M 的数据页加上 mysqld 自身
开销仍会超过 `MemoryHigh=384M`，照样被回收。**两边都要动**：

| | 改前 | 改后 | 依据 |
|---|---|---|---|
| `innodb_buffer_pool_size` | 1024M | **384M** | pool 里真正的数据页 315M；384M 是 chunk(128M) 的整数倍，留 22% 余量 |
| `MemoryHigh` | 384M | **768M** | pool 384M + 自身开销约 150M ≈ 550M，留余量 |
| `MemoryMax` | 640M | **1G** | 封顶防跑飞 |

**零停机怎么做到的**：三个应用服务都 `Requires=mysql97`，重启它会连锁停掉全部
（§13.1d 那次就是这么断的 81 秒）。而这两样都能在线改 ——
`systemctl set-property` 改 cgroup、`SET GLOBAL innodb_buffer_pool_size` 在线缩池。
**顺序不能反**：先缩池会在缩的过程中又撞一次上限。

| 验证 | 结果 |
|---|---|
| `memory.events` 的 `high` | **2941 → 2941 → 2941**，三分钟零增长（改前约 30 次/分钟） |
| cgroup 当前用量 | 415M，**不再顶在上限**（改前死顶 383M/384M） |
| mysqld RSS / Swap | 392M→427M / 434M→371M —— 页在自然回来 |
| 服务 | 四个全程 `active`、health 200、巡检非 OK 项 0 |

**落盘时收拾的两处「两个真相源」**：① 限额原本被 `systemctl set-property` 写进
`/etc/systemd/system.control/`（标着「Do not edit」、不进仓库），挪到可版本化的 drop-in；
② 基础单元里还留着旧的 `MemoryHigh=384M`（被 drop-in 覆盖才没生效），删掉。

**`swapoff` 在有 cgroup 限额的机器上不要再用** —— 正确的做法是把限额调对，
让页自己回来，而不是强行塞。

#### 13.2b 邮件通道接通了（2026-09-16 12:42）

收件人 `robin@neargo.ai`。**没有新增任何凭据** —— 巡检在运行时直接去读应用
已经在用的那套 M365 SMTP 配置（`shop-app.env` 里的 `MAIL_*`）。
理由：这台机器上已经有一套验证过能发信的凭据，再抄一份就是多一个要轮换的地方，
**而抄的过程本身就是一次泄露机会**。凭据由 `logwatch-mail.py` 自己逐行解析，
不经过 shell 变量、不进命令行、不落日志（`ps e` 也看不到）。

前置实测：587 出站**通**、25 被腾讯云封（所以本地 MTA 那条路走不通）、python3 3.12 带 smtplib。

| 验证 | 结果 |
|---|---|
| 压低阈值跑一轮真巡检 | `已发邮件 1 条 → robin@neargo.ai` |
| 消融①：同条件再跑一轮 | 无发信行 —— 6 小时去重生效 |
| 消融②：阈值调回正常 | 无 WARN、无发信 —— 不是「只要跑就发」 |
| **消融③：把 `MAIL_ENV_FILE` 指向不存在的文件** | 报 `读不到 ...` + `邮件发送失败（退出码 3）`，**且没有记「已推」** |
| 真实去重状态是否被测试污染 | `sent-*` 标记 **0** 条 |

**消融③是这里最要紧的一条。** 「发失败了却记成已推」会把一条真告警**永久吞掉六小时**，
而且没有任何症状 —— 比不发还坏。所以代码里是「有一条通道送达才 `touch` 标记」，
不是先记后送。

**标题自带结论**：`[CRIT] <主机> 巡检 · disk / 91%（警告 80% · 严重 90%）` ——
手机锁屏上只看得到标题，标题不说清就等于没告警。

**⚠️ 「SMTP 接收了」不等于「收件箱里有」**：M365 可能事后退信
（最常见是 `554 SendAsDenied`，当 `MAIL_FROM` 与 `MAIL_USERNAME` 不一致时）。
**要人确认收到了才算这条闭环。**

#### 13.2c 巡检加上了「服务还活着没有」（2026-09-16 12:50）

`logwatch.sh` 巡的是 `disk` / `log-total` / `rate` / `outbox-failed` / `binlog` ——
**全是存储指标，一项健康检查都没有**（对着脚本 grep `systemctl is-active|actuator/health` 命中 **0**，
对照量 3 处提到 `systemctl`/`curl`，证明尺子是活的）。

用 §13.1d 那次事故当判据：**12:29:16 起三个服务全停 81 秒，而 12:07 那轮巡检报的是一整屏 `OK`，
在那 81 秒里它报的还会是一整屏 `OK`** —— 服务死了，磁盘不涨、日志不涨。

更糟的是这一条：

```
OK   rate ai-shop/shop-app   1.5K/h（上限 50M/h）
```

服务一死，日志速率归零，在这把尺子上读作 `0.0K/h` = **OK**。
**死掉的服务比活着的服务看起来更健康。**

这是 9-14 那次教训被用偏了的结果：方案盯死了「上一次出事的那个东西」（磁盘），
没盯「最常出事的那个东西」（服务挂了）。

**已补第 4c 节**：五个服务 `systemctl is-active` + 两个 HTTP 探针
（shop-app `/actuator/health`=200 · pay-svc `/internal/pay/fee-rules`=401，
pay 没有 actuator，401 才是「容器起来了、过滤链在」的活口）+ MySQL `SELECT 1`，
任一不通报 **CRIT**。

**一小时一轮换不来「立刻知道」**，它换来的是把「挂了一整夜」变成「挂了一小时」——
而在这一节存在之前，那个数是「永远不知道」。

**失败复查一次再报**：部署本身会让服务离线十几秒，巡检正好撞上就报 CRIT 是误报，
而误报会训练人忽略这个通道 —— 比没有告警更坏（§11.2 同形状）。

##### 消融时抓到的一个真缺陷

前两个分支**一条 CRIT 都没报**，查下来不是漏判，是**脚本当场退出了**：
`set -euo pipefail` 下 `st="$(systemctl is-active "$u")"` 里的命令返回非零会让整个脚本死掉，
而 `is-active` 对「服务不在」正是返回非零，`curl` 连不上同理。

**也就是说：这个健康检查会在恰好有东西要报的那一刻悄悄死掉。** 已加 `|| true`。

另一处：`${VAR:-默认}` 在 `VAR=""` 时也取默认，于是消融里想单独关掉某一项时**关不掉**，
验的是别的对象。已改成 `${VAR-默认}`。
...
| | 结果 |
|---|---|
| 分支①服务不在 | `CRIT svc no-such-service **inactive**` |
| 分支②HTTP 连不上 | `CRIT http health **连不上**（期望 200）` |
| 分支③HTTP 通但码不对 | `CRIT http health **200**（期望 503）` |
| 分支④MySQL 查不动 | `CRIT mysql **连不上或查不动**` |
| 对照：真实目标 | CRIT **0** |
| **真实链路**：真停掉 `ai-shop-job` | `CRIT svc ai-shop-job **failed**` → **`已发邮件 1 条 → robin@neargo.ai`** |
| 恢复后复查 | 八项全 OK、CRIT 0 |
| 真实去重状态是否被污染 | `sent-*` **0** |

真实链路那次自停 **48 秒**（12:50:04 停 → 12:50:52 恢复）。替身太干净会盖住真缺陷，
而这一次确实盖不住 —— 见下。

##### 顺带查出：job-worker 收到 SIGTERM 不退出

`systemctl stop ai-shop-job` 花了 **45 秒**，最后是被 `SIGKILL` 强杀的：

```
ai-shop-job.service: Main process exited, code=killed, status=9/KILL
ai-shop-job.service: Failed with result 'timeout'.
```

对照 `ai-shop-pay` 的同类操作是 `Deactivated successfully`。三个单元都配了
`SuccessExitStatus=143`，所以不是单元配置问题，**是应用自己关不掉**。

后果有三个：① 每次发 job-worker 都白等 45 秒；② 在途任务被硬杀而不是排空 ——
这正好解释 12:00 那次部署为什么出 `CannotGetJdbcConnectionException /
HikariDataSource (job-pool) has been closed`；③ 正常停止之后单元停在 `failed` 而不是
`inactive`，计划内维护看起来像崩溃。

**未修，且已经有一次失败的尝试 —— 记下来免得再走一遍**：
同伴试过「关停时先停调度器」（`0d46aa57e`，给 `JobWorkerConfig` 加 `SmartLifecycle`），
真机实测**毫无效果**，已回退（`9934b9a86`）。回退说明里的日志时刻是决定性的：

```
11:59:27  job-pool - Shutdown initiated       ← 池在这里就关了
12:00:32  调度器已停止排期（新加的那行）        ← 晚了一分多钟
```

**Hikari 的关闭不走 Bean 销毁那条路**，所以「`SmartLifecycle.stop()` 在销毁单例之前跑，
能赶在池关之前」这个前提根本不成立。下一个人要修，得先搞清 Hikari 到底是被谁、
在哪个阶段关掉的，而不是再往关停顺序上加一层。
