...
**一句话现状（9-16 上午更新）：「写不满盘」做完了；「有人知道」已经修到只差一个 URL。**
上限（logback 封顶、journald 200M、binlog 7 天、logrotate）全部到位并验过；
推送链路 9-16 也端到端验过了（§13.2），**但 `logwatch.env` 里仍然没有 `WEBHOOK_URL` 这个键** ——
在有人把它粘上去之前，所有结果仍然只进 `logwatch.log` 与 journal，等于仍要靠人主动去看。
9-14 那次事故缺的正是这一环。
...
| **2** | **开两条告警通道**（云监控磁盘 + 企业微信机器人） | **人（只差一个 URL）** | 它是整份方案的目的本身；不开，下面所有上限出事时仍然没人知道 | **链路已在 9-16 证明可用**（见 §13.2），只差 `logwatch.env` 里的 `WEBHOOK_URL=` 一行。云监控那条仍待人在控制台开 |
| ✅ | 删历史不规范日志（§10.5 清单，实为 **14** 个） | 我 | 删完 `/data/log` 与 `/home/deploy`、`/tmp` 就只剩规范位置 | **已做**（9-16 10:35）：先打包验解压逐字节一致再删；14 个路径回读为 0，归档仍在。详见 §10.5 |
| **4** | 清理开删除（cron 加 `APPLY=1`） | 人开关，我可改配置 | 原计划 9-21。清单已连跑两天且内容干净可核（9-16 那轮：**将删 12 项 740MB**，全是 08-29/09-02/09-05 的过期部署包 + 3 份无标签 env 副本；认不出归属的三个 jar 只报不删）。`/tmp` 已从 838M 涨到 **982M**。**建议仍照原计划 9-21** —— 「先跑一周对清单」是自己定的闸，提前开等于这道闸从没起过作用 | 开后次日 04:10 看 housekeeping.log：那 12 项真被删、三个认不出的 jar 仍在 |
...
| ◐ | 把这批改动部署上去，再按同口径回量一次 | 我 | 提交不等于生效 | **已部署**（9-16 10:32，`shop-app-20260916-1032-76d260f9.jar`，health 200、0 ERROR）。**回量还没做** —— 要等 recon 跑够轮次。预期见 §7.4：WARN 从 **104** 降到 35–45 行 |
