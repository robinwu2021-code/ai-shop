...
**预期（部署 7.3 那两个提交之后才成立，现在线上跑的还是改前的 jar）**：
WARN 从 104 降到约 35–45 行，且**剩下的几乎全是「每次启动一条」的横幅** ——
也就是说稳态下出现一条 WARN 就真的意味着出事了。
**这条要回读验证**：部署后按同样的口径再量一次，降不到这个量级就是改法没生效。
...
**关 SSH 口令登录。** ⚠️ 陷阱：`sshd_config` 第 12 行先 Include 了 `sshd_config.d/50-cloud-init.conf`，
...
**一句话现状：「写不满盘」这半边做完了，「有人知道」这半边还没有。**
上限（logback 封顶、journald 200M、binlog 7 天、logrotate）全部到位并验过；
但**告警一条都推不出去** —— 所有结果只进 `logwatch.log` 与 journal，等于仍要靠人主动去看。
9-14 那次事故缺的正是这一环，它现在还缺着。

| # | 做什么 | 谁 | 为什么这个顺序 | 判据（能失败的） |
|---|---|---|---|---|
| **1** | **关 SSH 口令登录 + root 口令登录** | **人** | 最急。实测仍是 `passwordauthentication yes` / `permitrootlogin yes`，而公网每天上千次爆破。**已挂两天** | `sshd -T` 出 `no` / `prohibit-password`；新终端公钥能进、`-o PubkeyAuthentication=no` 被拒 |
| **2** | **开两条告警通道**（云监控磁盘 + 企业微信机器人） | **人** | 它是整份方案的目的本身；不开，下面所有上限出事时仍然没人知道 | 临时调低阈值跑一轮巡检，**群里真收到一条**；云监控发一次测试告警 |
| **3** | 删 15 个历史不规范日志（§10.5 清单） | **人** | 一条 `rm` 的事；删完 `/data/log` 与 `/home/deploy`、`/tmp` 就只剩规范位置 | `find` 回读那 15 个路径为 0 |
| **4** | 清理开删除（cron 加 `APPLY=1`） | 人开关，我可改配置 | 原计划 9-21。清单已连跑两天，`/tmp` 838M 的残留包一直列着没删 | 开后次日 04:10 看 housekeeping.log：`/tmp` 的 9 个包真被删、`legacy/` 仍只报不删 |
| **5** | 删兼容软链（6 个） | 人 | **门槛已过**（2026-09-16 实测）：13 个生效配置（三个单元 · 三条 cron · 两份 logrotate · 两份 nginx · 三个 env · ops 脚本）里旧路径 **0 处**；对照量非零（事故 README 里数出 5 处，证明这把尺不是空转）。三个软链的 atime 都还停在 09-14 15:0x —— 搬完之后没人走过旧路 | 删完三个服务各重启一轮仍 health 200；nginx `-t` 通过 |
...
| **7** | **把上面这批改动部署上去，再按同口径回量一次** | 我做，需你点 | **改了但没部署，线上一行都没少** —— 现在跑的还是改前的 jar。这是整批工作唯一还没有闭环的一环 | §7.4 的预期：WARN 从 **104** 降到 35–45 行，且剩下的几乎全是「每次启动一条」的横幅。降不到就是改法没生效 |

**7 是这批代码改动的闭环**：提交不等于生效。在部署之前，§7.4 那张表就是线上的现状。
