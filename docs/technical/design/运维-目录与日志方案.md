...
> 状态：**M0–M5 已实施 · 清理（只列清单）与巡检已装 · 复核后已修三处（见 §11）· 推送通道待人工配置 · 兼容软链未删** · 创建 2026-09-14 · 复核 2026-09-14
...
而且要先证明那一刻**真的发生过**断开重连，否则「0 条」什么也不说明。结果：检查挂在 22:33 之后跑，出来补在这里。
...
- **第一次按天滚动**（当晚 0 点）：次日看 `shop-app.log.2026-09-14.0.gz` 存在、`shop-app.log` 从 0 点重新开始写
