...
同一个异常反复发生只在第一次打完整堆栈（outbox 已做：投满 10 次转 FAILED、堆栈只打首尾两次，`6a375ac5`）· 每个异常最多 30 帧 ·
...
建议改法：**状态变化时打完整 WARN，未变化时降级为一行摘要（或每 N 轮打一次）**；
它同时也是真问题的信号（微信通道 21 笔查不通已持续多轮），修日志时别把问题一起消音。

**（b）`org.ehcache` 40 行不在 §7 的名单里。** 它是框架噪音里第二大的一块，
而生产 `SHOP_TOKEN_STORE=db`、本不该走 ehcache。要么把它加进规范③的名单，要么先查清它为什么还在初始化。
