...
### 8.3 纪律项：库里有、页面还是自己画了一份

**26 处 / 6 类**（建立基线时 33 处 / 7 类）。全部登记在
`known-handrolled-ui.txt`，第八道闸拦新增、并点名修好后忘了删的陈行。

| 形态 | 处 | 为什么还留着 |
|---|---:|---|
| 系统弹框 | 10 | `showActionSheet` 的「选一项」。**换 sh-sheet 是换交互形态不是换皮肤** |
| 选中态自画 | 6 | 剩下的都是不成族的独一份（下划线 tab / 步骤条 / 数字高亮） |
| 文字当图标 | 3 | 尺寸受宿主约束，与 sh-icon 的几何不是一回事 |
| 文字当箭头 | 3 | `›` 贴在文本节点尾部，换图标要动文本节点 |
| 分段标题 | 2 | **不缺组件，缺间距档** |
| 弹层 / 遮罩 | 2 | **2026-08-27 补判据才现形**：原规则只认 mask/dlg/modal/popup 四个类名，`.sheet` 不在其中，而它们同时被「白块自画」误报着 —— **一条规则的误命中会变成另一条规则的漏报，账面上还看着挺干净** |
