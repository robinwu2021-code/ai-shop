...
- **6 类是库里根本没有** —— 其中 **3 类是 2026-08-26 才拆出来的**：
  「选中态自画 13 页」这一个名字**盖住了开关（3 页）· 勾选框（4 页）· 选项卡片（4 页）
  三个缺件**（见 §十一）。另外三类是：列表行（32 页，§九已论证**不该做成组件**）·
  键值行（6 页，键宽要拍）· 搜索框（1 页，形态要拍）
...
1. **`sh-switch`**（3 页）· **`sh-check`**（4 页，单选/多选两态）· **`sh-option`**（4–5 页）
   —— 这三件是真缺件，做完「库的缺口」才算真的清空
2. `goods-edit` 的四处 chip 收回 `.sh-chip--primary`
   —— 其中三处是纯删代码，一处（规格档位）会去掉描边与加粗

**②的观感变化是可见的，而我没有办法在真机上验**（本机 dev server 连真后端，
登录要凭据）。①基本是等价替换：三个开关、四个勾选框各自本来就长得不一样，
统一之后至少一致。
