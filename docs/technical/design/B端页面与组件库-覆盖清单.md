...
| 行内首行 `.row__head` | 14 | 7 | **要定夺**：一半是 `gap`（紧挨着排），一半是 `space-between`（两端对齐），这是两种意图不是两种写法 |
...
| 3 | 行内首行到底是 `gap` 还是 `space-between`（§9.2）| 14 页 7 种写法，背后是两种意图。定了才谈得上收编 |
| 4 | `stores` 的「切店」用 600 加重（§9.3），与字阶「600 只给标题与按钮」冲突 | 要么承认「一屏里最常用的那一下」是一档，要么改用别的形态强调 |
| 5 | `goods-edit` 3 933 行 / 110 条选择器要不要拆 | 它一页占了商品域样式的四成。补完组件后应该能掉一大半，届时再看还剩什么 |
