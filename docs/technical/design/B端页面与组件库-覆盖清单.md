...
| 2 | **C 端的 `.field`** | C 端 6 个页面各自定义了一份，圆角 24/32、底色 faint/surface、字号 26/28/30 三处不一致，而 `base.css` 里 `.field__input` 已经有了。**`b-app/App.vue` 里那条注释说「C 端几乎没有表单」—— 这个前提是错的** |
...
**33 处 / 7 类**（B 端 61 页扫过）。**已全部登记进 `known-handrolled-ui.txt`**，
新增的会被第八道闸拦下；修好了要把那一行删掉，守卫会点名陈行。

| 形态 | 处 | 为什么还留着 |
|---|---:|---|
| 系统弹框 | 10 | `showActionSheet` 的「选一项」。**换 sh-sheet 是换交互形态不是换皮肤** —— 要整批动并逐页看过 |
| 白块自画 | 7 | 照抄了积木的声明，直接换类名即可。**这是最该改的一类**，只是要逐页核内边距 |
| 选中态自画 | 6 | 剩下的都是不成族的独一份（下划线 tab / 步骤条 / 数字高亮） |
| 文字当图标 | 3 | 尺寸受宿主约束，与 sh-icon 那套「给一个 box 值」的几何不是一回事 |
| 文字当箭头 | 3 | `›` 贴在文本节点尾部，换图标要先把文本拆成两个节点 |
| 分段标题 | 2 | **不缺组件，缺间距档** |
| 弹层 / 遮罩 | 2 | 动画时长与关闭手势和 sh-sheet 不同，换过去要对一遍手感 |

缺口类只剩 **列表行 39 页**，已论证不该做成组件（§九），**不进闸门** ——
拦它只会让每个新列表页被挡一次，而正确做法就是自己写。**恒红的闸门是噪声掩体。**
