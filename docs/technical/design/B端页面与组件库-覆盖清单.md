...
目标是「**所有页面都基于组件库开发**」。逐页扫完 54 页，离这个目标还差多少：

| | 数 |
|---|---|
| B 端页面 | **54** ← 收尾期间隔壁会话新加了进销存页面，一并算进来了 |
| 完全没有自造形态的页 | **13** |
| 自造形态实例 | **64 处 / 8 类** |
| 页面自己的样式 | ~~1 119 条选择器 · 5 583 行~~ → **1000 条 · 4915 行**（已含新页） |

**这 8 类要分成两堆看，责任完全不同：**

- **7 类是库里有、页面没用** —— 这是纪律问题，改页面就行（分栏切换、空态、＋加一项、系统弹框已清零或大幅收敛）
- **只剩 1 类是库里根本没有：列表行（32 页）—— 而它 §九 已论证不该做成组件。**
  也就是说「库的缺口」这一堆**清空了**。

> **2026-08-26 收尾：所有待决一次处理完**（见 §十二）。
> **§十一 那三个缺件已经补齐**：`sh-switch`（3 页）·
...
改这几类不需要新增任何组件，是**纯纪律**。**下表的页数是 2026-08-26 收尾后的实测值**，
括号里是收编前的原值 —— 留着原值是因为「降了多少」比「现在是多少」更说明问题。

| 形态 | 页数 | 库里对应 | 哪些页 |
|---|---:|---|---|
| **系统弹框** | ~~26~~ → **7** | `sh-sheet` | **`uni.showModal` 在 b-app 已归零**（带输入的 12 处走 `prompt()`，确认型 25 处走 `confirm()`）。剩下的 7 页是 `showActionSheet`（选一项），那是另一种形态：coupons, customers, goods-edit, marketing, member-reach, member-tags, picking |
| **选中态自画** | ~~16~~ → **6** | `.sh-chip--primary` | 「13 页」拆开是五种控件，三种是缺件（§十一，已补齐）。剩下 6 页：activity-edit, apply, cross-store, login, store-pick, verify |
...
| 形态 | 页 | 结论 |
|---|---:|---|
| 列表行 | 33 | **不该做成组件** —— 行的躯干是各页业务（`reviews` 一行八个条件块），见 §九。真正在重复的三小件已分别处置：文字动作收成 `.sh-link`、行尾动作排属间距档、行内首行是三件东西共用一个名字 |

原先并列的另两条已经落地：**键值行** → `sh-kv`（§十二.1）、**搜索框** → 判据撤掉（真正只有 1 处）。
...
> ⚠️ **这一节原先写错过一次，如实更正**：我当时只比了「选中态那一行」，
> 说「六处里有三处已经等于标准」。**比上基类之后只有一处是真的** ——
> 另外三处根本不在 `.sh-chip` 上，它们各自是**小方角**（16/16/24rpx），
> 换成 `.sh-chip` 会从方角变胶囊，那是可见变化不是删代码。

| 页 | 类 | 在 `.sh-chip` 上？ | 现在 | 处置 |
|---|---|---|---|---|
| `goods-edit` | `.param__chip--on` | ✅ 是（基类只多一条与 `.sh-chip` **相同**的 font-size） | tint + primary-text | ✅ **已删**（2026-08-26），零观感变化 |
| `goods-edit` | `.opt--on` | ✅ 是 | tint + primary-text + **2rpx 实线主色 + 600** | 待验 —— 去描边去 600 是可见变化 |
| `goods-edit` | `.opt--off` | ✅ 是 | faint + **2rpx 透明描边**（为了与选中等高） | 同上，两条一起改 |
| `goods-edit` | `.seg` | ❌ 否 | 自己一套：`8/20` + **16rpx 方角** | 见 11.4b |
| `goods-edit` | `.lang` | ❌ 否 | 自己一套：`8/18` + **16rpx 方角** + 选中加 600 | 见 11.4b |
| `store-notice` | `.ttl__i` | ❌ 否 | 自己一套：`10/20` + **24rpx 方角** | 见 11.4b |
| `cross-store` | `.tag--off` | ✅ 是 | 在 `.sh-chip` 上转灰 | 保留（这是「不可用」不是「未选中」） |

### 11.4b 冒出来的第四种药丸：小方角分段

`.seg` / `.lang` / `.ttl__i` 不是「写歪的 chip」，它们是**另一种形态**：
方角小块，用在「切换看哪一列 / 切哪一门语言 / 选标题模板」这类**分段**上。
问题是这三处**彼此也不一致**：

| | 内边距 | 圆角 |
|---|---|---|
| `goods-edit .seg` | `8rpx 20rpx` | 16rpx |
| `goods-edit .lang` | `8rpx 18rpx` | 16rpx |
| `store-notice .ttl__i` | `10rpx 20rpx` | **24rpx** |

**要拍的是**：小方角分段算不算一档？
算 → 收成 `.sh-chip--square` 之类，三处统一；
不算 → 三处都变胶囊走 `.sh-chip`，那是三页的可见变化。
**这条没有偷偷决定**，列进 §八 待决。
...
**② `goods-edit` 的 `.param__chip` 已删（零观感变化），`.opt--on/--off` 还没动** —— 其中三处是纯删代码，
...
## 十二、收尾：待决一次处理完 + H5 全量走查

### 12.1 七条待决的处置

| # | 待决 | 处置 |
|---|---|---|
| 1 | 规格档位去描边去 600 | ✅ 去掉，并把**多选**改用 `✓` 记号表达 —— 此前靠一句提示文案说，而**视觉重量指向了错的那件事**（规格更重，读起来像「更要紧」而不是「能多选」）。勾用的是这次补进 `icons.ts` 的 `check` |
| 2 | 小方角分段算不算一档 | ✅ **不算**。`.seg`/`.lang`/`.ttl__i` 三处都走 `.sh-chip` 胶囊 —— `sh-tabs` 的注释里写着方块那一套当初就是被统一掉的，为「卡内」开例外等于把它请回来 |
| 3 | 键值行键宽 140/180 | ✅ 收成 `sh-kv`，默认 160rpx（三处里两处是且逐字节相同），`keyWidth` 由调用点给 —— **列宽是版面不是件的属性**。两种排法（定宽键 / 两端对齐）**是两件事**，做成一个组件两个形态 |
| 4 | 搜索框两种形态 | ✅ **判据撤掉**：真正的只有 1 处（`customers` 的 `.search` 是手机号查询）。一处不构成组件 |
| 5 | `cross-store` 用虚线药丸标演示数据 | ✅ 改掉 —— 虚线药丸在这套界面里的意思是「候选：点一下就加」，拿它标「演示数据」是同一视觉两个意思 |
| 6 | 图标钮点按区 56rpx < 44pt | ✅ **补到 88rpx 而版面一点没动**：元素撑到 `box+32rpx`，再用 `margin:-16rpx` 从版面里减回去 —— 手指碰 88，排版占 56 |
| 7 | 25 处确认型 `showModal` | ✅ 全换成 `confirm()`。**`showModal` 在 b-app 归零。** 收益不只是字：危险操作终于有了自己那一档（`.sh-btn--danger-solid` 此前两端引用 **0**，清单每次都报「定义了没人用」—— 不是没人需要，是没有地方用它） |

### 12.2 三道设计守卫全绿

本会话开头它们红着 7 条断言。现在 **36 条全绿**（typography 7 + design-tokens 19 + skin-vars 1 + ui-package 9）。

**修的过程里又逮到两处判据自己的毛病**（同 `amt/due` 那次一课）：

- 「把主色当文字色用」报 9 个，**其中 7 个是 `border-color`** —— 正则 `(?<!-text)color:` 会命中 `border-color:` / `border-bottom-color:`。改成 `(?<![-\w])color:`，真违规只有 2 处
- `entities` 写的是 `var(--sh-ok, #22a06b)` / `var(--sh-warn, #e08b00)` —— **皮肤里根本没有这两个变量**（语义色叫 `--sh-success` / `--sh-warning`），于是一直落在兜底的硬编码色上：**换肤与深色模式都跟不上，而没有任何症状**

字号 17 处、圆角 11 处一并归档（22→24 / 32→34 / 36→34；8·12·20→16，28→32）。

### 12.3 H5 全量走查（自动）

在一台**临时**的 mock dev server（5179，跑完已停、配置已撤）上跑完，因为
**Vite 对软链进来的 `packages/ui` 不重新转译** —— 别的会话那台 5175 认不出新组件
（memory 里那条老坑，这次又验了一遍：服务端源码是新的，浏览器拿到的模块是旧的）。

| 项 | 结果 |
|---|---|
| 52 个页面逐页访问（走查当时的页数） | **0 处报错、0 处空白** |
| `prompt()` 端到端 | ✓ 弹出 · **初值进输入框** · 说明另起一行 · 取消收起 · **确定后写入真的生效**（改名后列表更新） |
| `confirm()` 端到端 | ✓ 弹出 · 说明单独一行 · **危险档用上 `.sh-btn--danger-solid`** · 取消收起 |
| 新组件渲染 | `sh-uploader`（主图角标 + 删除钮都吃 `--sh-scrim`）· `sh-kv` ×3 · `sh-section` ×5 · `sh-add` ×2 · `sh-switch` ×4 · `sh-option` ×2 · `sh-icon-btn` ×4 · `sh-tabs` ×2 · `sh-fab` ×1 |
| 文字箭头残留 | **0** |
| 深色 + 换肤 | ✓ 卡片 / 输入框 / 正文 / 新组件全部跟着皮肤走（无写死色） |

唯一的控制台报错是启动时两条 `setNavigationBarColor:fail page not found` ——
uni 自身在页面就绪前的固有行为，与本轮改动无关。

---

## 八、待决

> **这张表改过两次**（2026-08-26）。第一次是因为发现表里 8 条标着「待决」的其实
> 已经做掉了 —— **待决表自己过期，比没有待决表更糟**：它让人以为还有一堆没动，
> 真正开着的几条反倒被淹掉。第二次是这一轮又做掉了 5 条。

### 8.1 已处置

| 待决 | 结论 |
|---|---|
| `showModal` 要不要一刀切 | ✅ b-app 已归零：带输入 12 处 → `prompt()`，确认型 25 处 → `confirm()` |
| 数字格 40 还是 44 / 600 还是 700 | ✅ 40rpx / 600，依据是字阶不是投票 |
| 超大数字没有档 | ✅ 字阶加第八档 `.txt-mega`（60rpx） |
| 图标钮点按区只有 56rpx | ✅ 补到 88rpx 而版面没动（撑大 + 负外边距） |
| 小方角分段 / 虚线药丸 / 键值行键宽 / 搜索框 | ✅ 见上一版，判据与取舍都记在各自组件的注释里 |
| 守卫要不要进 `pre-push` | ✅ 已挂（第七道闸）。先修绿再挂闸 —— 恒红的闸门等于全员推不动 |
| **`.txt-*` 与 `.sh-h1/.sh-h2` 二选一** | ✅ **两者逐行相同**，一档两个名字。B 端 97 处改叫正名，`.sh-h1/.sh-h2` 的定义留着给 C 端那 30 处，那一轮改完即删 |
| **描边要不要认 hairline** | ✅ **认，但给的是变量不是积木类** —— 分隔线只有一条声明，而积木类是替掉元素的整体长相。43 处收进 `--sh-hairline` / `--sh-hairline-soft` |
| **`.field__head` 的归属** | ✅ 它定义在 `goods-edit` 自己的 style 里却顶着 `field__` 前缀，搬进 `base.css` 与同族团聚 |
| **立「新页面不许自造」的闸门** | ✅ **第八道闸**（`check-handrolled-ui.mjs` + `known-handrolled-ui.txt`）。判据复用生成器的 `ROLLED` 表，两处各写一份的话改了一处另一处照样绿 |
| **C 端 6 份 `.field` 定义** | ✅ **合一**：16 个输入框改用共用的 `.field__input`（address/me/groups/group-host/request-create），`login` 刻意的大控件改名 `.login__field` 留在页内。`b-app/App.vue` 里「C 端几乎没有表单」那句前提**是错的**，已订正 |
| **白块自画 7 处** | ✅ **整类归零**。2 处真抄了 `.sh-card`（messages 消息卡 / home 九宫格格子）已改用积木；另 5 处是**判据误命中** —— 原规则是「白底 + 任意圆角」，而库里的卡与块都是 lg(32) 四角统一 |
| ~~补齐 B 端的 `sh-block__head`~~ | ❌ **判断有误，撤销**：B 端 6 处并没有自画标题行，用的是 `sh-section` 组件。真正的事是 `sh-section` 与 `.sh-block__head` 该合成一件 —— 那是 C 端的 12 处，属下一轮 |

### 8.2 还开着的

**B 端已经没有开着的了。** 下面四条：两条有结论（记在这儿是因为「为什么不做」比
「做了什么」更容易被下一个人推翻重来），两条属于 C 端那一轮。

| # | 待决 | 结论 |
|---|---|---|
| 1 | **`goods-edit` 抽 composable** | ❌ **不抽**（2026-08-27）。逐节量过：3 868 行分成 14 节，**最大的一节 247 行**，没有庞然大物 —— 文件大是因为这一页做的事多，不是某一块乱。而仓库唯一的抽取先例 `utils/spec-override.ts` 注释写着「抽出来是因为它**有两个调用方**」，`useParams` 只有一个。抽出去不减复杂度，只是把它挪到另一个文件，还多一层注入依赖。**真正有效的是分节**（14 条注释，此前 script 里一条都没有）与把站错节的函数搬回去（已搬三个：`loadCategories`/`prunable`/`moreOther`）—— 分节标题说了假话就不如没有 |
| 6 | **`.txt-hero`（48rpx）引用为 0** | ✅ **保留，且清单改了报法**。字阶是一个**闭合集合**，不是使用清单：48rpx 在 tokens.ts 里是一档，`typography.test.ts` 也断言九个类齐全，删掉等于把 40 与 60 之间挖空，而下一个要 48 的人只会手写一个。生成器现在把「积木/组件没人用（真欠账）」与「字阶档位没调用点（允许）」分两句报 —— **混在一句里的后果是真欠账被一条恒在的噪声盖住**，`.txt-hero` 就这么挂了一整轮 |
| 5 | **`biz-region-picker` 1 628 行** | ❌ **建议不拆**。42 个函数围绕**一个状态机**（钻取路径 + 已选集合 + 覆盖判定），拆开等于把状态变成参数在三个文件间传。行数大在这里是「一件事确实复杂」，与 `goods-edit` 的「多件事挤在一起」不是同一个病 |
| 3·4 | **`sh-section` 与 `.sh-block__head` 合并**、**删 `.sh-h1/.sh-h2` 的定义** | ⏭ **C 端那一轮**。前者 C 端 12 处引用，后者 C 端还有 30 处（h1×3 / h2×27） |

### 8.3 纪律项：已归零

**33 处 / 7 类 → 0。** B 端 61 页扫过，一处不剩。
`known-handrolled-ui.txt` 现在是空的 —— 它从「已知欠账放行」变成了
**一处新的自造都不许进**，新增的会被第八道闸拦下并点名（实测会红且点名到页）。

清的过程里**判据本身改了四次，每一次都是同一个毛病：按名字或按形状归类**。
这一条比清掉的数量更值得记：

| 判据 | 原来 | 收窄成 | 它当时报的数 |
|---|---|---|---|
| 白块自画 | 白底 + 任意圆角 | lg(32) 且四角统一 | 7 → 真的只有 2 |
| 弹层 / 遮罩 | 只认 mask/dlg/modal/popup | 补上 `sheet` | 2 → 其实是 4，另 2 处被「白块」误报着 |
| 选中态自画 | `(--on\|--off\|is-on)` | 抄了 chip 的处理 | 6 → 真的只有 3 |
| 分段标题 | 类名叫不叫 `sec__h` | 标题自己带着 margin | 2 → 真的只有 2，但改完就该退场 |

**最值得记的一条**：那两个自画的底部弹层同时被「白块」误报着，
于是「弹层」那条的漏报在账面上看不出来 ——
**一条规则的误命中，替另一条规则的漏报打了掩护。**

而「文字当箭头」「文字当图标」两条判据**已撤下**，改由硬闸门守
（`packages/shared/tests/icon-chars.test.ts`，模板 + i18n 词条一起扫）：
它们只扫模板，而箭头大多藏在词条里 —— 判据报 3 处，真实是 30 处。

缺口类只剩 **列表行 39 页**，已论证不该做成组件（§九），**不进闸门** ——
拦它只会让每个新列表页被挡一次。

当前家底：61 页 · 30 个组件 · 44 个积木。

### 8.4 那处「没验到的」——已经验到了，但只验到一半

`goods-list` 的「修改库存」此前记着「自动走查里点不开」。2026-08-27 点开了：
**按钮确实触发 `editStock`**，多规格分支按设计跳编辑页（改哪个 SKU 说不清，
所以只对单规格开快捷入口）。所以「点不开」那句话是错的 —— 点击**没有**落到
整行的跳转上，是它自己跳的。

**仍然没走到的是单规格那条分支**（`prompt()` 改库存并回写）：
mock 里 8 个商品全是多规格，构造不出来。`prompt()` 本身是端到端验过的，
但**「这个按钮 + 单规格」这条具体的路没有跑过**，如实记着。

> 这一条留着是因为它演示了一种很常见的错法：**「验不了」与「验过了是另一个结果」
> 长得一样**。当时点击落空，我记下的是「点不开」——而真相是它工作正常、
> 只是走了另一条分支。判断一件事没被验证，比判断它坏了要难。
