...
| # | 待决 | 现状 |
|---|---|---|
| 1 | **`goods-edit` 抽 composable** | 3 768 行，script 占 2 166 行 / 63 个函数。这一轮先补了 **14 条分节注释**（此前一条都没有），并把站错队的 `loadCategories`/`prunable` 搬回「类目」节。整体拆分单独一轮：只抽 `useSpecs`（16 函数）与 `useParams`（7 函数），类目/图片/价格与主表单状态耦合深，抽出去要把 `form` 穿来穿去 |
| 3 | **`sh-section` 与 `.sh-block__head`** | 同一个想法在两个容器里的两套实现。C 端 12 处引用后者，合并属 C 端那一轮 |
| 4 | **`.sh-h1/.sh-h2` 的定义什么时候删** | C 端还有 30 处（h1×3 / h2×27） |
| 5 | **`biz-region-picker` 1 628 行** | **建议不拆**：42 个函数围绕一个状态机（钻取路径 + 已选集合 + 覆盖判定），拆开等于把状态变成参数在三个文件间传。行数大在这里是「一件事确实复杂」，不是「三件事挤在一起」 |
| 6 | **`.txt-hero`（48rpx）引用为 0** | B 端七处 48rpx 都不是这一档，见标准库清单的说明。要么 C 端在用，要么这一档该删 —— 但不能按数字硬凑 |
