...
| 3 | **数字格用 40 还是 44、600 还是 700** | ✅ **已定：40rpx / 600**。不是投票 —— 字阶写着「700 只给价格」，而这四处是发放数 / 触达数 / 会员数 / 命中数，一个都不是价格；44rpx 也不在七档上。40/600 恰好是 `customers` 现在的值：四处里唯一合规的那一个 |
...
| 5 | **超大数字（60 / 72rpx）没有档** | 新浮现。`income`/`points` 的 `.amt` 是 60rpx、`order` 的应收 `.due` 是 72rpx，而字阶最大一档 `.txt-hero` 只有 48rpx。它们都是「老板照着它收钱」的那个数，诉求真实 —— 要么给字阶加一档，要么承认 48 够用。**这条没有偷偷决定** |
