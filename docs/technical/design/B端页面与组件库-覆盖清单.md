...
> **这张表 2026-08-26 重写过一次** —— 上一版里有 5 条标着「待决」，其实已经做掉了。
> 待决表自己过期，比没有待决表更糟：它会让人以为还有一堆没动。

### 8.1 已处置（留着，因为「怎么定的」比「定成什么」有用）

| 原待决 | 结论 |
|---|---|
| `showModal` 要不要一刀切 | ✅ **b-app 已归零**：带输入 12 处 → `prompt()`，确认型 25 处 → `confirm()` |
| 数字格 40 还是 44 / 600 还是 700 | ✅ **40rpx / 600**，依据是字阶不是投票 |
| `stores` 的「切店」用 600 加重 | ✅ **换形态不加字重**，顺带补了 `.sh-btn--sm` |
| 超大数字没有档 | ✅ **字阶加第八档 `.txt-mega`**（60rpx），规范正文已同步 |
| 图标钮点按区只有 56rpx | ✅ **补到 88rpx 而版面没动**（撑大 + 负外边距） |
| 小方角分段算不算一档 | ✅ **不算**，三处都走 `.sh-chip` 胶囊 |
| `cross-store` 用虚线药丸标演示数据 | ✅ 改掉 —— 虚线的约定意思是「候选：点一下就加」 |
| 键值行键宽 140/180 | ✅ 收成 `sh-kv`，默认 160，`keyWidth` 由调用点给 |
| 搜索框两种形态 | ✅ 判据撤掉 —— 真正只有 1 处，不构成组件 |
| 守卫要不要进 `pre-push` | ✅ **已挂（第七道闸）**。先修绿再挂闸 —— 恒红的闸门挂上去等于全员推不动 |

### 8.2 还开着的

| # | 待决 | 现状与两个方向 |
|---|---|---|
| 1 | **立「新页面不许自造」的闸门** | 判据现成（生成器里的 `ROLLED` 表），而且**现在正是时候**：缺件已清空，剩下的 64 处按棘轮登记为已知欠账，新增的拦下。不立的话，这一轮收编会像 `sh-tabs` 的方块 tab 一样慢慢长回来 |
| 2 | **`.txt-*` 与 `.sh-h1/.sh-h2` 二选一** | 字阶九个类两端合计只有 7 处引用，实际在跑的是 `.sh-h2`（B73/C27）。要么把字阶用起来，要么承认 `.sh-h*` 是真实字阶、把 `.txt-*` 删掉。**留着一套没人用的字阶，等于规范在说一件代码里不存在的事** |
| 3 | **描边：认不认 hairline** | 51 处 / 25 文件（会话开始 67/26）。**其中 27 处已经是 `2rpx solid var(--sh-line)`** —— 提议：承认它是第 13 条，收进 `.sh-hairline`，把另外 24 处（`2rpx faint`×7 / `1rpx line`×5 / `3rpx`×2 / `4rpx transparent`×2 …）归过来 |
| 4 | **`.sh-block` 在 B 端要不要落地** | B 端 2 处 / C 端 12 处。B 端密度更高，`.sh-card` 平铺可能就是对的 —— 那就把十二条第 10 条改成「C 端适用」，别让它一直挂着 |
| 5 | **`goods-edit` 要不要拆** | 3 749 行 / 81 条选择器（收编前 3 933 行 / 110 条）。降了但仍是全项目最大的一页 |
| 6 | **`biz-region-picker` 1 628 行** | 一个组件顶三个页面。拆成省市区 / 小区村 / 提报三段，还是它本来就该是一页 |
| 7 | **`.field__*` 的归属** | `.field__label` / `__input` / `__area` 定义在两端共用的 `base.css`，而 **C 端引用全是 0**；同族的 `.field` 又定义在 `b-app/App.vue`。一套表单件横跨两个文件、只有一端用 —— 要么整体挪进 B 端，要么整体留在共用层 |

### 8.3 剩下的纪律项（不需要拍板，只需要有人改）

「库里有、页面没用」共 7 类 64 处，按体量：

| 形态 | 页 | 怎么改 |
|---|---:|---|
| 列表行 | 33 | **不改** —— §九已论证不该做成组件 |
| `showActionSheet` | 7 | 10 处「选一项」。`sh-sheet` 能装，但那是**换交互形态**不是换皮肤，要先想清楚 |
| 白块自画 | 7 | 页面重画了 `.sh-card` / `.sh-block` 的形状，直接换类名 |
| 选中态自画 | 6 | 剩下的都是**不成族的独一份**（下划线 tab / 步骤条 / 数字高亮），收编只会多几个单调用点的件 |
| 分段标题 | 4 | **不缺组件，缺间距档** —— 各页的 margin 是真实的版面决定 |
| 文字当图标 | 3 | 输入框清空 ×3 / chip 内嵌 ×1 / 图片角标 ×1，**尺寸受宿主约束，是另一套几何** |
| 文字当箭头 | 2 | `marketing` / `store-scope` 的 `›` **贴在文字尾**（`{{ 店名 }} ›`），换图标要动文本节点 |

### 8.4 一处没验到的

`goods-list` 的「修改库存」按钮，自动走查里点不开（点击落到整行的跳转上）。
它的正确性靠的是**逐页对数**（37 → 37，动作无丢失）与 `prompt()` 的端到端验证，
