...
### 补丁 · 会员页在 App 里没有入口（2026-09-19 真机发现）

装 0.4.71 真机验证时发现：会员名单、标签详情、人群详情、给会员发消息这一圈页面只在彼此之间互链，
**工作台、我的、营销一处都不链进来** —— 批 B 起商家就点不到打标签与发消息，只能从「发出去的 → 效果页」绕进人群页。
「页面有没有门」闸门没拦住：它只问「有没有别的页面提到我」，一圈互链全部过关。

- 营销首页加一行「会员」→ `pages/customers/index`（有 `biz:customer` 才显示）
- `scripts/check-app-orphan.mjs` 加第二道：以 tab 页与全局代码为起点沿链接图走一遍，走不到的报出来；
  组件里的链接只在用到它的页面可达时才算。基线 `known-unreachable-pages.txt`（只准变短）。
